...
def ler_definicoes_registros(arquivo_markdown):
    """
    Lê o arquivo markdown e extrai as definições de todos os registros do bloco 9.
    
    Args:
        arquivo_markdown: Caminho para o arquivo sped_bloco_9.md
        
    Returns:
        Dicionário com código do registro como chave e lista de campos como valor
        Exemplo: {'9001': [{'numero': '01', 'campo': 'REG', 'tipo': 'C', ...}, ...]}
    """
    ...
    padrao_registro = r'REGISTRO\s+(9\d{3}):'
    ...
    for i, match in enumerate(matches):
        ...
        padrao_cabecalho = r'Nº\s+Campo\s+Descrição\s+Tipo\s+Tam\s+Dec\s+Obrig'
...
def parse_registro_generico(linha, codigo_registro, campos_definicao):
    ...
    Args:
        ...
        codigo_registro: Código do registro (ex: '9001')
...
def ler_bloco_9_do_sped(caminho_arquivo_sped):
    """
    Lê o arquivo SPED e extrai todas as linhas do bloco 9.
    Retorna uma lista de linhas do bloco 9.
    """
    linhas_bloco_9 = []
    
    if not os.path.exists(caminho_arquivo_sped):
        print(f"Erro: Arquivo SPED '{caminho_arquivo_sped}' não encontrado!")
        return linhas_bloco_9
    ...
    try:
        for linha in arquivo:
            ...
            # Verifica se é uma linha do bloco 9 (registros que começam com 9)
            # Formato: |9001|... ou |9900|... etc
            if linha.startswith('|') and '|' in linha[1:]:
                partes = linha.split('|')
                if len(partes) > 1 and partes[1] and partes[1].startswith('9'):
                    linhas_bloco_9.append(linha)
    finally:
        if arquivo:
            arquivo.close()
...
def processar_bloco_9(sped_arquivo=None):
    """
    Função principal que processa o bloco 9 do arquivo SPED.
    
    Fluxo:
    1. Lê o arquivo SPED e extrai linhas do bloco 9
    ...
    5. Processa todos os registros do bloco 9
    ...
    print("=" * 60)
    print("Parser do Bloco 9 - SPED EFD Fiscal")
    print("=" * 60)
    ...
    # 3. Buscar arquivo markdown da documentação do bloco 9
    caminho_markdown = base_dir / "documentacao_blocos" / versao / "bloco_9.md"
    ...
    # 5. Ler o bloco 9 do arquivo SPED
    print(f"\nLendo bloco 9 do arquivo SPED...")
    linhas_bloco_9 = ler_bloco_9_do_sped(str(caminho_sped))
    
    if not linhas_bloco_9:
        print("Aviso: Nenhuma linha do bloco 9 encontrada no arquivo SPED!")
        return
    
    print(f"Encontradas {len(linhas_bloco_9)} linhas do bloco 9")
    ...
    print("\nProcessando registros do bloco 9...")
    print("=" * 60)
    
    # 7. Processar cada tipo de registro encontrado no bloco 9
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    registros_processados = {}
    
    # Agrupa linhas por tipo de registro
    for linha in linhas_bloco_9:
        partes = linha.split('|')
...
if __name__ == "__main__":
    sped_arquivo = "08589276000169-9084073269-20201101-20201130-0-911F0BE8FE2F193E99CEFD2FA579B4E7F40C8ED9-SPED-EFD.txt"
    processar_bloco_9(sped_arquivo)
