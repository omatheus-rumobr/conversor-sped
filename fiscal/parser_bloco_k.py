...
def ler_definicoes_registros(arquivo_markdown):
    """
    Lê o arquivo markdown e extrai as definições de todos os registros do bloco K.
    
    Args:
        arquivo_markdown: Caminho para o arquivo sped_bloco_k.md
        
    Returns:
        Dicionário com código do registro como chave e lista de campos como valor
        Exemplo: {'K001': [{'numero': '01', 'campo': 'REG', 'tipo': 'C', ...}, ...]}
    """
    ...
    padrao_registro = r'REGISTRO\s+(K\d{3}):'
...
def parse_registro_generico(linha, codigo_registro, campos_definicao):
    ...
    Args:
        ...
        codigo_registro: Código do registro (ex: 'K001')
...
if __name__ == "__main__":
    base_dir = Path(__file__).parent
    caminho_markdown = base_dir / "docs" / "sped_bloco_k.md"
    diretorio_bloco_k = base_dir / "blocos_separados" / "bloco_K"
    
    print("=" * 60)
    print("Parser Genérico do Bloco K - SPED EFD Fiscal")
    print("=" * 60)
    ...
    for codigo_registro, campos_definicao in definicoes.items():
        caminho_arquivo = diretorio_bloco_k / f"{codigo_registro}.txt"
        
        print(f"\nProcessando registro {codigo_registro}...")
        registros = ler_arquivo_registro(str(caminho_arquivo), codigo_registro, campos_definicao)
        
        if registros:
            nome_arquivo_excel = base_dir / f"registro_{codigo_registro}_{timestamp}.xlsx"
            criar_arquivo_excel_registro(registros, codigo_registro, campos_definicao, str(nome_arquivo_excel), dados_bloco_0, participantes)
        else:
