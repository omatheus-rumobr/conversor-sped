...
def ler_definicoes_registros(arquivo_markdown):
    """
    Lê o arquivo markdown e extrai as definições de todos os registros do bloco H.
    
    Args:
        arquivo_markdown: Caminho para o arquivo sped_bloco_h.md
        
    Returns:
        Dicionário com código do registro como chave e lista de campos como valor
        Exemplo: {'H001': [{'numero': '01', 'campo': 'REG', 'tipo': 'C', ...}, ...]}
    """
    ...
    # Busca por padrões de registro: "REGISTRO H001:" ou "## REGISTRO H001:"
    padrao_registro = r'(?:##\s+)?REGISTRO\s+(H\d{3}):'
...
def parse_registro_generico(linha, codigo_registro, campos_definicao):
    ...
    Args:
        ...
        codigo_registro: Código do registro (ex: 'H001')
...
def obter_descricao_campo(campo, valor):
    ...
    descricoes = {
        ...
        'MOT_INV': {
            '01': '01 - No final no período',
            '02': '02 - Na mudança de forma de tributação da mercadoria (ICMS)',
            '03': '03 - Na solicitação da baixa cadastral, paralisação temporária e outras situações',
            '04': '04 - Na alteração de regime de pagamento – condição do contribuinte',
            '05': '05 - Por determinação dos fiscos',
            '06': '06 - Para controle das mercadorias sujeitas ao regime de substituição tributária – restituição/ ressarcimento/ complementação'
        }
    }
...
def aplicar_formatacao(campo, valor, tipo_campo):
    ...
    if campo.startswith('DT_') or campo.startswith('DTE_') or campo.startswith('DT_INV'):
...
def criar_arquivo_excel_registro(registros, codigo_registro, campos_definicao, nome_arquivo=None, dados_bloco_0=None, participantes=None):
    ...
    for registro in registros:
        ...
        cod_part = registro.get('COD_PART', '') or registro.get('COD_PART_IP', '') or registro.get('COD_PART_IT', '')
...
def ler_bloco_h_do_sped(caminho_arquivo_sped):
    """
    Lê o arquivo SPED e extrai todas as linhas do bloco H.
    Retorna uma lista de linhas do bloco H.
    """
    linhas_bloco_h = []
    
    if not os.path.exists(caminho_arquivo_sped):
        print(f"Erro: Arquivo SPED '{caminho_arquivo_sped}' não encontrado!")
        return linhas_bloco_h
    ...
    try:
        for linha in arquivo:
            ...
            # Verifica se é uma linha do bloco H (registros que começam com H)
            # Formato: |H001|... ou |H005|... etc
            if linha.startswith('|') and '|' in linha[1:]:
                partes = linha.split('|')
                if len(partes) > 1 and partes[1] and partes[1].upper().startswith('H'):
                    linhas_bloco_h.append(linha)
    finally:
        if arquivo:
            arquivo.close()
...
def extrair_datas_do_registro_0000(caminho_arquivo_sped):
    """
    Extrai as datas DT_INI e DT_FIN do registro 0000 do arquivo SPED.
...
def processar_bloco_h(sped_arquivo=None):
    """
    Função principal que processa o bloco H do arquivo SPED.
    
    Fluxo:
    1. Lê o arquivo SPED e extrai linhas do bloco H
    ...
    5. Processa todos os registros do bloco H
    ...
    print("=" * 60)
    print("Parser do Bloco H - SPED EFD Fiscal")
    print("=" * 60)
    print(f"\nLendo arquivo SPED: {caminho_sped}")
    
    # 1. Extrair datas DT_INI e DT_FIN do registro 0000 (do bloco 0)
    dt_ini_str, dt_fin_str = extrair_datas_do_registro_0000(str(caminho_sped))
    ...
    # 2. Buscar versão correspondente no versoes.json
    ...
    # 3. Buscar arquivo markdown da documentação do bloco H
    caminho_markdown = base_dir / "documentacao_blocos" / versao / "bloco_H.md"
    ...
    # 4. Ler definições dos registros do markdown
    ...
    # 5. Ler o bloco H do arquivo SPED
    print(f"\nLendo bloco H do arquivo SPED...")
    linhas_bloco_h = ler_bloco_h_do_sped(str(caminho_sped))
    
    if not linhas_bloco_h:
        print("Aviso: Nenhuma linha do bloco H encontrada no arquivo SPED!")
        return
    
    print(f"Encontradas {len(linhas_bloco_h)} linhas do bloco H")
    
    # 6. Ler dados do bloco 0 e participantes
    ...
    print("\nProcessando registros do bloco H...")
    print("=" * 60)
    
    # 7. Processar cada tipo de registro encontrado no bloco H
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    registros_processados = {}
    
    # Agrupa linhas por tipo de registro
    for linha in linhas_bloco_h:
        partes = linha.split('|')
        
        # O primeiro campo após o pipe inicial é o código do registro
        if len(partes) > 1:
            codigo_registro = partes[1].strip().upper()
...
if __name__ == "__main__":
    sped_arquivo = "08589276000169-9084073269-20201101-20201130-0-911F0BE8FE2F193E99CEFD2FA579B4E7F40C8ED9-SPED-EFD.txt"
    processar_bloco_h(sped_arquivo)
