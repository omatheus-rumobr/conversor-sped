...
def ler_definicoes_registros(arquivo_markdown):
    """
    Lê o arquivo markdown e extrai as definições de todos os registros do bloco G.
    
    Args:
        arquivo_markdown: Caminho para o arquivo sped_bloco_g.md
        
    Returns:
        Dicionário com código do registro como chave e lista de campos como valor
        Exemplo: {'G001': [{'numero': '01', 'campo': 'REG', 'tipo': 'C', ...}, ...]}
    """
    ...
    # Busca por padrões de registro: "REGISTRO G001:" ou "## REGISTRO G001:"
    padrao_registro = r'(?:##\s+)?REGISTRO\s+(G\d{3}):'
...
def parse_registro_generico(linha, codigo_registro, campos_definicao):
    ...
    Args:
        ...
        codigo_registro: Código do registro (ex: 'G001')
...
def ler_bloco_g_do_sped(caminho_arquivo_sped):
    """
    Lê o arquivo SPED e extrai todas as linhas do bloco G.
    Retorna uma lista de linhas do bloco G.
    """
    linhas_bloco_g = []
    
    if not os.path.exists(caminho_arquivo_sped):
        print(f"Erro: Arquivo SPED '{caminho_arquivo_sped}' não encontrado!")
        return linhas_bloco_g
    ...
    try:
        for linha in arquivo:
            ...
            # Verifica se é uma linha do bloco G (registros que começam com G)
            # Formato: |G001|... ou |G110|... etc
            if linha.startswith('|') and '|' in linha[1:]:
                partes = linha.split('|')
                if len(partes) > 1 and partes[1] and partes[1].upper().startswith('G'):
                    linhas_bloco_g.append(linha)
    finally:
        if arquivo:
            arquivo.close()
...
def processar_bloco_g(sped_arquivo=None):
    """
    Função principal que processa o bloco G do arquivo SPED.
    
    Fluxo:
    1. Lê o arquivo SPED e extrai linhas do bloco G
    ...
    5. Processa todos os registros do bloco G
    ...
    print("=" * 60)
    print("Parser do Bloco G - SPED EFD Fiscal")
    print("=" * 60)
    ...
    # 3. Buscar arquivo markdown da documentação do bloco G
    caminho_markdown = base_dir / "documentacao_blocos" / versao / "bloco_G.md"
    ...
    # 5. Ler o bloco G do arquivo SPED
    print(f"\nLendo bloco G do arquivo SPED...")
    linhas_bloco_g = ler_bloco_g_do_sped(str(caminho_sped))
    
    if not linhas_bloco_g:
        print("Aviso: Nenhuma linha do bloco G encontrada no arquivo SPED!")
        return
    
    print(f"Encontradas {len(linhas_bloco_g)} linhas do bloco G")
    ...
    print("\nProcessando registros do bloco G...")
    print("=" * 60)
    
    # 7. Processar cada tipo de registro encontrado no bloco G
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    registros_processados = {}
    
    # Agrupa linhas por tipo de registro
    for linha in linhas_bloco_g:
        partes = linha.split('|')
...
if __name__ == "__main__":
    sped_arquivo = "08589276000169-9084073269-20201101-20201130-0-911F0BE8FE2F193E99CEFD2FA579B4E7F40C8ED9-SPED-EFD.txt"
    processar_bloco_g(sped_arquivo)
