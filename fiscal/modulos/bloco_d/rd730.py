...
def _processar_linha_d730(linha):
    """
    Processa uma única linha do registro D730 e retorna um dicionário.
    
    Args:
        linha: String com uma linha do SPED no formato |D730|CST_ICMS|CFOP|ALIQ_ICMS|VL_OPR|VL_BC_ICMS|VL_ICMS|VL_RED_BC|COD_OBS|
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |D730|...|)
    ...
    # Nota: A validação do primeiro caractere do CFOP baseada em IND_OPER do D700
    # não pode ser feita aqui pois não temos acesso ao registro pai
    if not cfop:
        return None
    ...
    resultado = {
        ...
        "VL_ICMS": {
            "titulo": "Parcela correspondente ao \"Valor do ICMS\" referente à combinação CST_ICMS, CFOP, e alíquota do ICMS, incluindo o FCP, quando aplicável",
            "valor": vl_icms,
...
def validar_d730_fiscal(linhas):
    ...
    Args:
        linhas: String com uma linha, string com múltiplas linhas separadas por \\n,
                ou lista de strings. Cada linha deve estar no formato |D730|CST_ICMS|CFOP|ALIQ_ICMS|VL_OPR|VL_BC_ICMS|VL_ICMS|VL_RED_BC|COD_OBS|
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_d730(linha)
