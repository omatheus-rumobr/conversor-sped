...
def _processar_linha_d696(linha):
    """
    Processa uma única linha do registro D696 e retorna um dicionário.
    
    Args:
        linha: String com uma linha do SPED no formato |D696|CST_ICMS|CFOP|ALIQ_ICMS|VL_OPR|VL_BC_ICMS|VL_ICMS|VL_BC_ICMS_UF|VL_ICMS_UF|VL_RED_BC|COD_OBS|
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |D696|...|)
    ...
    vl_bc_icms_uf = obter_campo(7)
    vl_icms_uf = obter_campo(8)
    ...
    # CFOP: obrigatório, 4 dígitos, primeiro caractere deve ser 5 ou 6
    ...
    # Validação de VL_BC_ICMS, ALIQ_ICMS e VL_ICMS baseada no CST_ICMS
    if cst_icms_ultimos_dois in ["30", "40", "41", "50", "60"]:
        ...
        if aliq_icms and abs(aliq_icms_float) > 0.001:
            return None
        ...
        if aliq_icms:
            aliq_icms_valido_temp, aliq_icms_float_temp, _ = validar_valor_numerico(aliq_icms, decimais=2, obrigatorio=False, nao_negativo=True)
            if not aliq_icms_valido_temp:
                return None
        ...
        if not aliq_icms:
            return None
        aliq_icms_valido_temp, aliq_icms_float_temp, _ = validar_valor_numerico(aliq_icms, decimais=2, obrigatorio=True, positivo=True)
        if not aliq_icms_valido_temp:
            return None
    ...
    # VL_BC_ICMS_UF: obrigatório, numérico com 2 decimais, não negativo
    vl_bc_icms_uf_valido, vl_bc_icms_uf_float, _ = validar_valor_numerico(vl_bc_icms_uf, decimais=2, obrigatorio=True, nao_negativo=True)
    if not vl_bc_icms_uf_valido:
        return None
    
    # VL_ICMS_UF: obrigatório, numérico com 2 decimais, não negativo
    vl_icms_uf_valido, vl_icms_uf_float, _ = validar_valor_numerico(vl_icms_uf, decimais=2, obrigatorio=True, nao_negativo=True)
    if not vl_icms_uf_valido:
        return None
    
    # VL_RED_BC: obrigatório condicional
    # Se CST_ICMS terminar em 20 ou 70, deve ser maior que zero
    # Para outros CST_ICMS, pode ser zero ou maior que zero
    if cst_icms_ultimos_dois in ["20", "70"]:
        vl_red_bc_valido, vl_red_bc_float, _ = validar_valor_numerico(vl_red_bc, decimais=2, obrigatorio=True, positivo=True)
        if not vl_red_bc_valido:
            return None
    else:
        # Para outros CST_ICMS, VL_RED_BC deve ser >= 0
    ...
    resultado = {
        ...
        "VL_ICMS": {
            "titulo": "Parcela correspondente ao \"Valor do ICMS\" referente à combinação CST_ICMS, CFOP, e alíquota do ICMS",
            "valor": vl_icms,
            "valor_formatado": formatar_valor_monetario(vl_icms_float)
        },
        "VL_BC_ICMS_UF": {
            "titulo": "Parcela correspondente ao valor da base de cálculo do ICMS de outras UFs, referente à combinação de CST_ICMS, CFOP e alíquota do ICMS",
            "valor": vl_bc_icms_uf,
            "valor_formatado": formatar_valor_monetario(vl_bc_icms_uf_float)
        },
        "VL_ICMS_UF": {
            "titulo": "Parcela correspondente ao valor do ICMS de outras UFs, referente à combinação de CST_ICMS, CFOP, e alíquota do ICMS",
            "valor": vl_icms_uf,
            "valor_formatado": formatar_valor_monetario(vl_icms_uf_float)
        },
        "VL_RED_BC": {
...
def validar_d696(linhas):
    ...
    Args:
        linhas: String com uma linha, string com múltiplas linhas separadas por \\n,
                ou lista de strings. Cada linha deve estar no formato |D696|CST_ICMS|CFOP|ALIQ_ICMS|VL_OPR|VL_BC_ICMS|VL_ICMS|VL_BC_ICMS_UF|VL_ICMS_UF|VL_RED_BC|COD_OBS|
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_d696(linha)
