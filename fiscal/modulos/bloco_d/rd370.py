...
def _processar_linha_d370(linha):
    """
    Processa uma única linha do registro D370 e retorna um dicionário.
    
    Args:
        linha: String com uma linha do SPED no formato |D370|COD_MUN_ORIG|VL_SERV|QTD_BILH|VL_BC_ICMS|VL_ICMS|
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |D370|...|)
    ...
    qtd_bilh = obter_campo(3)
    vl_bc_icms = obter_campo(4)
    vl_icms = obter_campo(5)
    ...
    # QTD_BILH: obrigatório, numérico inteiro, maior que zero
    if not qtd_bilh:
        return None
    try:
        qtd_bilh_int = int(float(qtd_bilh))  # Converte para int, mas permite float como entrada
        if qtd_bilh_int <= 0:
            return None
        # Verifica se não tem casas decimais (deve ser inteiro)
        if '.' in qtd_bilh and float(qtd_bilh) != qtd_bilh_int:
            return None
    ...
    resultado = {
        ...
        "QTD_BILH": {
            "titulo": "Quantidade de bilhetes emitidos",
            "valor": qtd_bilh
        },
        "VL_BC_ICMS": {
...
def validar_d370_fiscal(linhas):
    """
    Valida uma ou mais linhas do registro D370 do SPED EFD Fiscal.
    
    Args:
        linhas: String com uma linha, string com múltiplas linhas separadas por \\n,
                ou lista de strings. Cada linha deve estar no formato |D370|COD_MUN_ORIG|VL_SERV|QTD_BILH|VL_BC_ICMS|VL_ICMS|
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_d370(linha)
