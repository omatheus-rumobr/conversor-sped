...
def _processar_linha_d190(linha):
    """
    Processa uma única linha do registro D190 e retorna um dicionário.
    
    Args:
        linha: String com uma linha do SPED no formato |D190|CST_ICMS|CFOP|ALIQ_ICMS|VL_OPR|VL_BC_ICMS|VL_ICMS|VL_RED_BC|COD_OBS|
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |D190|...|)
    ...
    # CST_ICMS: obrigatório, 3 dígitos, primeiro caractere sempre será Zero
    if not cst_icms:
        return None
    ...
    if len(cfop) != 4 or not cfop.isdigit():
        return None
    if cfop.endswith("00") or cfop.endswith("50"):
        return None
    
    # ALIQ_ICMS: opcional condicional, numérico com 2 decimais
    ...
    # VL_OPR: obrigatório, numérico com 2 decimais, não negativo
    vl_opr_valido, vl_opr_float, _ = validar_valor_numerico(vl_opr, decimais=2, obrigatorio=True, nao_negativo=True)
    ...
    # Validação: só pode ser preenchido se os dois últimos dígitos do CST_ICMS forem iguais a 20 ou 70
    cst_icms_ultimos_2 = cst_icms[1:3]
    if cst_icms_ultimos_2 not in ["20", "70"]:
        # Se CST_ICMS não termina em 20 ou 70, VL_RED_BC deve ser zero ou vazio
        if vl_red_bc:
            ...
            if not vl_red_bc_valido:
                return None
            if vl_red_bc_float != 0.0:
                return None
        vl_red_bc_float = 0.0
    else:
        # Se CST_ICMS termina em 20 ou 70, VL_RED_BC pode ser preenchido
        vl_red_bc_valido, vl_red_bc_float, _ = validar_valor_numerico(vl_red_bc, decimais=2, obrigatorio=False, nao_negativo=True)
        if not vl_red_bc_valido:
            return None
    ...
    resultado = {
        ...
        "VL_OPR": {
            "titulo": "Valor da operação correspondente à combinação de CST_ICMS, CFOP, e alíquota do ICMS",
            "valor": vl_opr,
...
def validar_d190_fiscal(linhas):
    """
    Valida uma ou mais linhas do registro D190 do SPED EFD Fiscal.
    
    Args:
        linhas: String com uma linha, string com múltiplas linhas separadas por \\n,
                ou lista de strings. Cada linha deve estar no formato |D190|CST_ICMS|CFOP|ALIQ_ICMS|VL_OPR|VL_BC_ICMS|VL_ICMS|VL_RED_BC|COD_OBS|
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_d190(linha)
