...
def _processar_linha_d170(linha):
    """
    Processa uma única linha do registro D170 e retorna um dicionário.
    
    Args:
        linha: String com uma linha do SPED no formato |D170|COD_PART_CONSG|COD_PART_RED|COD_MUN_ORIG|COD_MUN_DEST|OTM|IND_NAT_FRT|VL_LIQ_FRT|VL_GRIS|VL_PDG|VL_OUT|VL_FRT|VEIC_ID|UF_ID|
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |D170|...|)
    ...
    cod_mun_orig = obter_campo(3)
    cod_mun_dest = obter_campo(4)
    otm = obter_campo(5)
    ind_nat_frt = obter_campo(6)
    vl_liq_frt = obter_campo(7)
    vl_gris = obter_campo(8)
    vl_pdg = obter_campo(9)
    vl_out = obter_campo(10)
    vl_frt = obter_campo(11)
    veic_id = obter_campo(12)
    ...
    # OTM: obrigatório, numérico, 8 dígitos
    if not otm:
        return None
    if not otm.isdigit() or len(otm) != 8:
        return None
    
    # IND_NAT_FRT: obrigatório, valores válidos: ["0", "1"]
    if ind_nat_frt not in ["0", "1"]:
        return None
    ...
    # VL_GRIS: opcional condicional, numérico com 2 decimais, não negativo
    vl_gris_valido, vl_gris_float, _ = validar_valor_numerico(vl_gris, decimais=2, obrigatorio=False, nao_negativo=True)
    if not vl_gris_valido:
        return None
    
    # VL_PDG: opcional condicional, numérico com 2 decimais, não negativo
    vl_pdg_valido, vl_pdg_float, _ = validar_valor_numerico(vl_pdg, decimais=2, obrigatorio=False, nao_negativo=True)
    if not vl_pdg_valido:
        return None
    
    # VL_OUT: opcional condicional, numérico com 2 decimais, não negativo
    ...
    resultado = {
        ...
        "COD_PART_RED": {
            "titulo": "Código do participante (redespachante)",
        ...
        "OTM": {
            "titulo": "Registro do operador de transporte multimodal",
            "valor": otm
        },
        "IND_NAT_FRT": {
            "titulo": "Indicador da natureza do frete",
            "valor": ind_nat_frt,
            "descricao": ind_nat_frt_desc.get(ind_nat_frt, "")
        },
        "VL_LIQ_FRT": {
        ...
        "VL_GRIS": {
            "titulo": "Valor do gris (gerenciamento de risco)",
            "valor": vl_gris if vl_gris else "",
            "valor_formatado": formatar_valor_monetario(vl_gris_float) if vl_gris else ""
        },
        "VL_PDG": {
            "titulo": "Somatório dos valores de pedágio",
            "valor": vl_pdg if vl_pdg else "",
            "valor_formatado": formatar_valor_monetario(vl_pdg_float) if vl_pdg else ""
        },
        "VL_OUT": {
...
def validar_d170_fiscal(linhas):
    """
    Valida uma ou mais linhas do registro D170 do SPED EFD Fiscal.
    
    Args:
        linhas: String com uma linha, string com múltiplas linhas separadas por \\n,
                ou lista de strings. Cada linha deve estar no formato |D170|COD_PART_CONSG|COD_PART_RED|COD_MUN_ORIG|COD_MUN_DEST|OTM|IND_NAT_FRT|VL_LIQ_FRT|VL_GRIS|VL_PDG|VL_OUT|VL_FRT|VEIC_ID|UF_ID|
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_d170(linha)
