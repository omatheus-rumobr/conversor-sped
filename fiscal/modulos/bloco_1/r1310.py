...
def _processar_linha_1310(linha):
    """
    Processa uma única linha do registro 1310 e retorna um dicionário.
    
    Args:
        linha: String com uma linha do SPED no formato |1310|NUM_TANQUE|ESTQ_ABERT|VOL_ENTR|VOL_DISP|VOL_SAIDAS|ESTQ_ESCR|VAL_AJ_PERDA|VAL_AJ_GANHO|FECH_FISICO|
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |1310|...|)
    ...
    num_tanque = obter_campo(1)
    estq_abert = obter_campo(2)
    vol_entr = obter_campo(3)
    vol_disp = obter_campo(4)
    vol_saidas = obter_campo(5)
    estq_escr = obter_campo(6)
    val_aj_perda = obter_campo(7)
    val_aj_ganho = obter_campo(8)
    fech_fisico = obter_campo(9)
    ...
    resultado = {
        ...
        "FECH_FISICO": {
            "titulo": "Volume aferido no tanque, em litros. Estoque de fechamento físico do tanque",
            "valor": fech_fisico,
...
def validar_1310_fiscal(linhas):
    """
    Valida uma ou mais linhas do registro 1310 do SPED EFD Fiscal.
    
    Args:
        linhas: String com uma linha, string com múltiplas linhas separadas por \\n,
                ou lista de strings. Cada linha deve estar no formato |1310|NUM_TANQUE|ESTQ_ABERT|VOL_ENTR|VOL_DISP|VOL_SAIDAS|ESTQ_ESCR|VAL_AJ_PERDA|VAL_AJ_GANHO|FECH_FISICO|
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_1310(linha)
