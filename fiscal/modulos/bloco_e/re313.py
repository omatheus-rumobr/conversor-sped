...
def _processar_linha_e313(linha):
    """
    Processa uma única linha do registro E313 e retorna um dicionário.

    Formato:
      |E313|COD_PART|COD_MOD|SER|SUB|NUM_DOC|CHV_DOCe|DT_DOC|COD_ITEM|VL_AJ_ITEM|
    """
    ...
    chv_doce = obter_campo(6)
    dt_doc = obter_campo(7)
    cod_item = obter_campo(8)
    vl_aj_item = obter_campo(9)
    ...
    # Exceção: quando COD_MOD = 63 (BP-e), COD_PART não deve ser informado
    if cod_mod == "63":
        if cod_part:
            return None
    else:
        # Para outros modelos, COD_PART é opcional mas se informado deve ter até 60 caracteres
    ...
    # CHV_DOCe: opcional condicional, 44 dígitos
    if chv_doce:
        if len(chv_doce) != 44 or not chv_doce.isdigit():
            return None

        # Quando se tratar de NF-e/CT-e/CT-e OS/BP-e, conferir DV
        if cod_mod in ["55", "57", "67", "63"]:
