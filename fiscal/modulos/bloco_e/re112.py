...
def _processar_linha_e112(linha):
    """
    Processa uma única linha do registro E112 e retorna um dicionário.
    ...
    # NUM_DA: opcional condicional (manual depende do contexto do ajuste)
    if num_da and not num_da.strip():
        return None

    # NUM_PROC: opcional, até 60 caracteres
    ...
    # PROC: opcional (se informado, não pode ser só espaços)
    if proc and not proc.strip():
        return None

    # TXT_COMPL: opcional (se informado, não pode ser só espaços)
...
def validar_e112_fiscal(linhas):
    """
    Valida uma ou mais linhas do registro E112 do SPED EFD Fiscal.
