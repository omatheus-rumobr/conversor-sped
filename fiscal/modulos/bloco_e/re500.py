...
def _processar_linha_e500(linha, dt_ini_0000=None, dt_fin_0000=None):
    """
    Processa uma única linha do registro E500 e retorna um dicionário.
    ...
    Regras (manual 3.1.8):
    - REG deve ser "E500"
    - IND_APUR deve ser 0 (Mensal) ou 1 (Decendial)
    ...
    ind_apur = obter_campo(1)
    dt_ini = obter_campo(2)
    dt_fin = obter_campo(3)

    # IND_APUR: obrigatório, valores válidos [0, 1]
    if ind_apur not in ["0", "1"]:
        return None
    ...
    # Quando ambos informados, garante que período do E500 está contido no 0000
...
def validar_e500_fiscal(linhas, dt_ini_0000=None, dt_fin_0000=None):
    """
    Valida uma ou mais linhas do registro E500 do SPED EFD Fiscal.
    ...
    for l in linhas_para_processar:
        r = _processar_linha_e500(l, dt_ini_0000=dt_ini_0000, dt_fin_0000=dt_fin_0000)
        if r is not None:
            resultados.append(r)
