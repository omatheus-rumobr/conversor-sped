...
def ler_definicoes_registros(arquivo_markdown):
    """
    Lê o arquivo markdown e extrai as definições de todos os registros do bloco D.
    
    Args:
        arquivo_markdown: Caminho para o arquivo sped_bloco_d.md
        
    Returns:
        Dicionário com código do registro como chave e lista de campos como valor
        Exemplo: {'D001': [{'numero': '01', 'campo': 'REG', 'tipo': 'C', ...}, ...]}
    """
    ...
    # Busca por padrões de registro: "REGISTRO D001:" ou "## REGISTRO D001:"
    padrao_registro = r'(?:##\s+)?REGISTRO\s+(D\d{3}):'
...
def parse_registro_generico(linha, codigo_registro, campos_definicao):
    ...
    Args:
        ...
        codigo_registro: Código do registro (ex: 'D001')
...
def ler_bloco_d_do_sped(caminho_arquivo_sped):
    """
    Lê o arquivo SPED e extrai todas as linhas do bloco D.
    Retorna uma lista de linhas do bloco D.
    """
    linhas_bloco_d = []
    
    if not os.path.exists(caminho_arquivo_sped):
        print(f"Erro: Arquivo SPED '{caminho_arquivo_sped}' não encontrado!")
        return linhas_bloco_d
    ...
    try:
        for linha in arquivo:
            ...
            # Verifica se é uma linha do bloco D (registros que começam com D)
            # Formato: |D001|... ou |D100|... etc
            if linha.startswith('|') and '|' in linha[1:]:
                partes = linha.split('|')
                if len(partes) > 1 and partes[1] and partes[1].upper().startswith('D'):
                    linhas_bloco_d.append(linha)
    finally:
        if arquivo:
            arquivo.close()
...
def processar_bloco_d(sped_arquivo=None):
    """
    Função principal que processa o bloco D do arquivo SPED.
    
    Fluxo:
    1. Lê o arquivo SPED e extrai linhas do bloco D
    ...
    5. Processa todos os registros do bloco D
    ...
    print("=" * 60)
    print("Parser do Bloco D - SPED EFD Fiscal")
    print("=" * 60)
    ...
    # 3. Buscar arquivo markdown da documentação do bloco D
    caminho_markdown = base_dir / "documentacao_blocos" / versao / "bloco_D.md"
    ...
    # 5. Ler o bloco D do arquivo SPED
    print(f"\nLendo bloco D do arquivo SPED...")
    linhas_bloco_d = ler_bloco_d_do_sped(str(caminho_sped))
    
    if not linhas_bloco_d:
        print("Aviso: Nenhuma linha do bloco D encontrada no arquivo SPED!")
        return
    
    print(f"Encontradas {len(linhas_bloco_d)} linhas do bloco D")
    ...
    print("\nProcessando registros do bloco D...")
    print("=" * 60)
    
    # 7. Processar cada tipo de registro encontrado no bloco D
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    registros_processados = {}
    
    # Agrupa linhas por tipo de registro
    for linha in linhas_bloco_d:
        partes = linha.split('|')
...
if __name__ == "__main__":
    sped_arquivo = "08589276000169-9084073269-20201101-20201130-0-911F0BE8FE2F193E99CEFD2FA579B4E7F40C8ED9-SPED-EFD.txt"
    processar_bloco_d(sped_arquivo)
