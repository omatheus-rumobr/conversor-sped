...
def ler_definicoes_registros(arquivo_markdown):
    """
    Lê o arquivo markdown e extrai as definições de todos os registros do bloco B.
    
    Args:
        arquivo_markdown: Caminho para o arquivo sped_bloco_b.md
        
    Returns:
        Dicionário com código do registro como chave e lista de campos como valor
        Exemplo: {'B001': [{'numero': '01', 'campo': 'REG', 'tipo': 'C', ...}, ...]}
    """
    ...
    padrao_registro = r'REGISTRO\s+(B\d{3}):'
...
def parse_registro_generico(linha, codigo_registro, campos_definicao):
    ...
    Args:
        ...
        codigo_registro: Código do registro (ex: 'B001')
...
def ler_bloco_b_do_sped(caminho_arquivo_sped):
    """
    Lê o arquivo SPED e extrai todas as linhas do bloco B.
    Retorna uma lista de linhas do bloco B.
    """
    linhas_bloco_b = []
    
    if not os.path.exists(caminho_arquivo_sped):
        print(f"Erro: Arquivo SPED '{caminho_arquivo_sped}' não encontrado!")
        return linhas_bloco_b
    ...
    try:
        for linha in arquivo:
            ...
            # Verifica se é uma linha do bloco B (registros que começam com B)
            # Formato: |B001|... ou |B020|... etc
            if linha.startswith('|') and '|' in linha[1:]:
                partes = linha.split('|')
                if len(partes) > 1 and partes[1] and partes[1].upper().startswith('B'):
                    linhas_bloco_b.append(linha)
    finally:
        if arquivo:
            arquivo.close()
...
def processar_bloco_b(sped_arquivo=None):
    """
    Função principal que processa o bloco B do arquivo SPED.
    
    Fluxo:
    1. Lê o arquivo SPED e extrai linhas do bloco B
    ...
    5. Processa todos os registros do bloco B
    ...
    print("=" * 60)
    print("Parser do Bloco B - SPED EFD Fiscal")
    print("=" * 60)
    ...
    # 3. Buscar arquivo markdown da documentação do bloco B
    # Tenta diferentes variações do nome do arquivo
    caminho_markdown = base_dir / "documentacao_blocos" / versao / "bloco_B.md"
    if not caminho_markdown.exists():
        caminho_markdown = base_dir / "documentacao_blocos" / versao / "bloco_b.md"
    
    if not caminho_markdown.exists():
        print(f"Erro: Arquivo de documentação não encontrado. Tentou:")
        print(f"  - {base_dir / 'documentacao_blocos' / versao / 'bloco_B.md'}")
        print(f"  - {base_dir / 'documentacao_blocos' / versao / 'bloco_b.md'}")
        return
    ...
    # 5. Ler o bloco B do arquivo SPED
    print(f"\nLendo bloco B do arquivo SPED...")
    linhas_bloco_b = ler_bloco_b_do_sped(str(caminho_sped))
    
    if not linhas_bloco_b:
        print("Aviso: Nenhuma linha do bloco B encontrada no arquivo SPED!")
        return
    
    print(f"Encontradas {len(linhas_bloco_b)} linhas do bloco B")
    ...
    print("\nProcessando registros do bloco B...")
    print("=" * 60)
    
    # 7. Processar cada tipo de registro encontrado no bloco B
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    registros_processados = {}
    
    # Agrupa linhas por tipo de registro
    for linha in linhas_bloco_b:
        partes = linha.split('|')
...
if __name__ == "__main__":
    sped_arquivo = "08589276000169-9084073269-20201101-20201130-0-911F0BE8FE2F193E99CEFD2FA579B4E7F40C8ED9-SPED-EFD.txt"
    processar_bloco_b(sped_arquivo)
