...
def ler_definicoes_registros(arquivo_markdown):
    """
    Lê o arquivo markdown e extrai as definições de todos os registros do bloco E.
    
    Args:
        arquivo_markdown: Caminho para o arquivo sped_bloco_e.md
        
    Returns:
        Dicionário com código do registro como chave e lista de campos como valor
        Exemplo: {'E001': [{'numero': '01', 'campo': 'REG', 'tipo': 'C', ...}, ...]}
    """
    ...
    # Busca por padrões de registro: "REGISTRO E001:" ou "## REGISTRO E001:"
    padrao_registro = r'(?:##\s+)?REGISTRO\s+(E\d{3}):'
...
def parse_registro_generico(linha, codigo_registro, campos_definicao):
    ...
    Args:
        ...
        codigo_registro: Código do registro (ex: 'E001')
...
def ler_bloco_e_do_sped(caminho_arquivo_sped):
    """
    Lê o arquivo SPED e extrai todas as linhas do bloco E.
    Retorna uma lista de linhas do bloco E.
    """
    linhas_bloco_e = []
    
    if not os.path.exists(caminho_arquivo_sped):
        print(f"Erro: Arquivo SPED '{caminho_arquivo_sped}' não encontrado!")
        return linhas_bloco_e
    ...
    try:
        for linha in arquivo:
            ...
            # Verifica se é uma linha do bloco E (registros que começam com E)
            # Formato: |E001|... ou |E100|... etc
            if linha.startswith('|') and '|' in linha[1:]:
                partes = linha.split('|')
                if len(partes) > 1 and partes[1] and partes[1].upper().startswith('E'):
                    linhas_bloco_e.append(linha)
    finally:
        if arquivo:
            arquivo.close()
...
def processar_bloco_e(sped_arquivo=None):
    """
    Função principal que processa o bloco E do arquivo SPED.
    
    Fluxo:
    1. Lê o arquivo SPED e extrai linhas do bloco E
    ...
    5. Processa todos os registros do bloco E
    ...
    print("=" * 60)
    print("Parser do Bloco E - SPED EFD Fiscal")
    print("=" * 60)
    ...
    # 3. Buscar arquivo markdown da documentação do bloco E
    caminho_markdown = base_dir / "documentacao_blocos" / versao / "bloco_E.md"
    ...
    # 5. Ler o bloco E do arquivo SPED
    print(f"\nLendo bloco E do arquivo SPED...")
    linhas_bloco_e = ler_bloco_e_do_sped(str(caminho_sped))
    
    if not linhas_bloco_e:
        print("Aviso: Nenhuma linha do bloco E encontrada no arquivo SPED!")
        return
    
    print(f"Encontradas {len(linhas_bloco_e)} linhas do bloco E")
    ...
    print("\nProcessando registros do bloco E...")
    print("=" * 60)
    
    # 7. Processar cada tipo de registro encontrado no bloco E
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    registros_processados = {}
    
    # Agrupa linhas por tipo de registro
    for linha in linhas_bloco_e:
        partes = linha.split('|')
...
if __name__ == "__main__":
    sped_arquivo = "08589276000169-9084073269-20201101-20201130-0-911F0BE8FE2F193E99CEFD2FA579B4E7F40C8ED9-SPED-EFD.txt"
    processar_bloco_e(sped_arquivo)
