...
def _processar_linha_c830(linha):
    """
    Processa uma única linha do registro C830 e retorna um dicionário.
    
    Formato:
      |C830|NUM_PROC|IND_PROC|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "C830"
    ...
    Uma vez procedida à escrituração do Registro "C830", deve a pessoa jurídica gerar os registros
    "1010" ou "1020" referente ao detalhamento do processo judicial ou do processo administrativo,
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |C830|...|)
    ...
    resultado = {
        "REG": {
            "titulo": "Registro",
            "valor": reg,
            "descricao": "Texto fixo contendo 'C830'"
        },
        "NUM_PROC": {
...
def validar_c830(linhas):
    """
    Valida uma ou mais linhas do registro C830 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_c830(linha)
