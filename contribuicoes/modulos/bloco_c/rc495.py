...
def _processar_linha_c495(linha):
    """
    Processa uma única linha do registro C495 e retorna um dicionário.
    
    Formato:
      |C495|COD_ITEM|CST_COFINS|CFOP|VL_ITEM|VL_BC_COFINS|ALIQ_COFINS|QUANT_BC_COFINS|ALIQ_COFINS_QUANT|VL_COFINS|COD_CTA|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "C495"
    ...
    - CFOP: opcional, código fiscal de operação e prestação (4 dígitos)
    ...
    Nota: Este registro detalha a consolidação de documentos emitidos por ECF (Códigos 02, 2D, 59 e 60) – Cofins.
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |C495|...|)
    ...
    quant_bc_cofins = obter_campo(7)
    aliq_cofins_quant = obter_campo(8)
    vl_cofins = obter_campo(9)
...
def validar_c495(linhas):
    """
    Valida uma ou mais linhas do registro C495 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_c495(linha)
