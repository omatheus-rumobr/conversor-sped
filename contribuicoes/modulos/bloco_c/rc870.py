...
def _processar_linha_c870(linha):
    """
    Processa uma única linha do registro C870 e retorna um dicionário.
    
    Formato:
      |C870|COD_ITEM|CFOP|VL_ITEM|VL_DESC|CST_PIS|VL_BC_PIS|ALIQ_PIS|VL_PIS|CST_COFINS|VL_BC_COFINS|ALIQ_COFINS|VL_COFINS|COD_CTA|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "C870"
    ...
    SAT-CF-e, segmentado por CST (CST PIS/Pasep e CST Cofins) ou por item.
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |C870|...|)
...
def validar_c870(linhas):
    """
    Valida uma ou mais linhas do registro C870 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_c870(linha)
