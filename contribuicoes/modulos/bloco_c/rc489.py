...
def _processar_linha_c489(linha):
    """
    Processa uma única linha do registro C489 e retorna um dicionário.
    
    Formato:
      |C489|NUM_PROC|IND_PROC|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "C489"
    ...
    Uma vez procedida à escrituração do Registro "C489", deve a pessoa jurídica gerar os registros
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |C489|...|)
    ...
    resultado = {
        "REG": {
            "titulo": "Registro",
            "valor": reg,
            "descricao": "Texto fixo contendo 'C489'"
        },
        "NUM_PROC": {
...
def validar_c489(linhas):
    """
    Valida uma ou mais linhas do registro C489 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_c489(linha)
