...
def _processar_linha_c485(linha):
    """
    Processa uma única linha do registro C485 e retorna um dicionário.
    
    Formato:
      |C485|CST_COFINS|VL_ITEM|VL_BC_COFINS|ALIQ_COFINS|QUANT_BC_COFINS|ALIQ_COFINS_QUANT|VL_COFINS|COD_ITEM|COD_CTA|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "C485"
    ...
    a Cofins incidente sobre as vendas por ECF, por item vendido no período.
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |C485|...|)
    ...
    quant_bc_cofins = obter_campo(5)
    aliq_cofins_quant = obter_campo(6)
...
def validar_c485(linhas):
    """
    Valida uma ou mais linhas do registro C485 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_c485(linha)
