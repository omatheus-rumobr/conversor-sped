...
def _processar_linha_c185(linha):
    """
    Processa uma única linha do registro C185 e retorna um dicionário.
    
    Formato:
      |C185|CST_COFINS|CFOP|VL_ITEM|VL_DESC|VL_BC_COFINS|ALIQ_COFINS|QUANT_BC_COFINS|ALIQ_COFINS_QUANT|VL_COFINS|COD_CTA|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "C185"
    ...
    da Cofins referentes a cada item objeto de venda por Nota Fiscal Eletrônica – NF-e.
    
    Deve ser informado um registro C185 para cada CST, CFOP ou Alíquotas, referentes às vendas do item no
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |C185|...|)
...
def validar_c185(linhas):
    """
    Valida uma ou mais linhas do registro C185 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_c185(linha)
