...
def _processar_linha_c609(linha):
    """
    Processa uma única linha do registro C609 e retorna um dicionário.
    
    Formato:
      |C609|NUM_PROC|IND_PROC|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "C609"
    ...
    para a adequada validação das contribuições sociais ou de créditos.
    
    Uma vez procedida à escrituração do Registro "C609", deve a pessoa jurídica gerar os registros
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |C609|...|)
    ...
    resultado = {
        "REG": {
            "titulo": "Registro",
            "valor": reg,
            "descricao": "Texto fixo contendo 'C609'"
        },
        "NUM_PROC": {
...
def validar_c609(linhas):
    """
    Valida uma ou mais linhas do registro C609 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_c609(linha)
