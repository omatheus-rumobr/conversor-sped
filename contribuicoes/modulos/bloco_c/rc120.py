...
def _processar_linha_c120(linha):
    """
    Processa uma única linha do registro C120 e retorna um dicionário.
    
    Formato:
      |C120|COD_DOC_IMP|NUM_DOC_IMP|VL_PIS_IMP|VL_COFINS_IMP|NUM_ACDRAW|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "C120"
    ...
    - VL_PIS_IMP: opcional, valor pago de PIS na importação (numérico, 2 decimais)
      - Informar o valor recolhido de PIS/Pasep – Importação, relacionado ao documento informado
    - VL_COFINS_IMP: opcional, valor pago de COFINS na importação (numérico, 2 decimais)
    ...
    documentadas pela nota fiscal escriturada no registro C100 (registro individualizado de documentos
    códigos 1, 1B, 04 e 55 - NF-e), quando o campo IND_OPER for igual a "0" (zero), indicando operação
    de entrada e que no registro filho C170 conste CST_PIS ou CST_COFINS gerador de crédito (CST 50 a 56),
    bem como conste no registro C170 CFOP próprio de operações de importação (CFOP iniciado em 3).
    
    Este registro deve ser utilizado mesmo que o documento escriturado em C100 e C170 refira-se a NFe
    (modelo 55). O registro C199 somente deve ser utilizado quando a PJ optou por relacionar suas operações
    com direito a crédito e documentadas por NFe nos registros consolidadores C190 e filhos.
    
    Não podem ser informados para um mesmo documento fiscal, dois ou mais registros com o mesmo conteúdo
    no campo NUM_DOC_IMP e NUM_ACDRAW (validação em camada superior).
    ...
    preencher o Registro "C120" para validar a apuração do crédito.
    
    Deve ser informado neste registro os pagamentos de PIS/Pasep-Importação e de Cofins-Importação,
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |C120|...|)
...
def validar_c120(linhas):
    """
    Valida uma ou mais linhas do registro C120 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_c120(linha)
