...
def _processar_linha_c195(linha):
    """
    Processa uma única linha do registro C195 e retorna um dicionário.
    
    Formato:
      |C195|CNPJ_CPF_PART|CST_COFINS|CFOP|VL_ITEM|VL_DESC|VL_BC_COFINS|ALIQ_COFINS|QUANT_BC_COFINS|ALIQ_COFINS_QUANT|VL_COFINS|COD_CTA|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "C195"
    ...
    consolidados de Cofins referentes a cada item objeto de aquisição e/ou devolução, por Nota Fiscal
    Eletrônica – NF-e.
    
    Deve ser informado um registro C195 para cada CST, CFOP ou Alíquotas, referentes às aquisições e devoluções
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |C195|...|)
    ...
    quant_bc_cofins = obter_campo(8)
    aliq_cofins_quant = obter_campo(9)
    ...
    # CST_COFINS: obrigatório, código da situação tributária COFINS (valores válidos para créditos)
...
def validar_c195(linhas):
    """
    Valida uma ou mais linhas do registro C195 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_c195(linha)
