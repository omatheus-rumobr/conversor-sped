...
def _processar_linha_c605(linha):
    """
    Processa uma única linha do registro C605 e retorna um dicionário.
    
    Formato:
      |C605|CST_COFINS|VL_ITEM|VL_BC_COFINS|ALIQ_COFINS|VL_COFINS|COD_CTA|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "C605"
    ...
    Nota: Este registro detalha as informações referentes a COFINS, consolidadas no registro C600.
    
    No caso de a pessoa jurídica auferir receitas com regimes tributários (CST-COFINS) distintos, deve a pessoa jurídica
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |C605|...|)
    ...
    vl_bc_cofins = obter_campo(3)
    aliq_cofins = obter_campo(4)
    vl_cofins = obter_campo(5)
    ...
    resultado = {
        "REG": {
            "titulo": "Registro",
            "valor": reg,
            "descricao": "Texto fixo contendo 'C605'"
        },
        "CST_COFINS": {
...
def validar_c605(linhas):
    """
    Valida uma ou mais linhas do registro C605 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_c605(linha)
