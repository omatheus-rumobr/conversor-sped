...
def _processar_linha_c820(linha):
    """
    Processa uma única linha do registro C820 e retorna um dicionário.
    
    Formato:
      |C820|CFOP|VL_ITEM|COD_ITEM|CST_PIS|QUANT_BC_PIS|ALIQ_PIS_QUANT|VL_PIS|CST_COFINS|QUANT_BC_COFINS|ALIQ_COFINS_QUANT|VL_COFINS|COD_CTA|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "C820"
    ...
    ou por CST (CST PIS/Pasep e CST Cofins), correspondente a receitas tributadas por quantidade de produtos vendidos.
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |C820|...|)
    ...
    resultado = {
        "REG": {
            "titulo": "Registro",
            "valor": reg,
            "descricao": "Texto fixo contendo 'C820'"
        },
        "CFOP": {
...
def validar_c820(linhas):
    """
    Valida uma ou mais linhas do registro C820 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_c820(linha)
