...
def _processar_linha_c601(linha):
    """
    Processa uma única linha do registro C601 e retorna um dicionário.
    
    Formato:
      |C601|CST_PIS|VL_ITEM|VL_BC_PIS|ALIQ_PIS|VL_PIS|COD_CTA|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "C601"
    ...
    Nota: Este registro detalha as informações referentes ao PIS/Pasep, consolidadas no registro C600.
    
    No caso de a pessoa jurídica auferir receitas com regimes tributários (CST-PIS) distintos, deve a pessoa jurídica
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |C601|...|)
...
def validar_c601(linhas):
    """
    Valida uma ou mais linhas do registro C601 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_c601(linha)
