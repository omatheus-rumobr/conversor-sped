...
def _processar_linha_c890(linha):
    """
    Processa uma única linha do registro C890 e retorna um dicionário.
    
    Formato:
      |C890|NUM_PROC|IND_PROC|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "C890"
    ...
    Uma vez procedida à escrituração do Registro "C890", deve a pessoa jurídica gerar os registros
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |C890|...|)
...
def validar_c890(linhas):
    """
    Valida uma ou mais linhas do registro C890 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_c890(linha)
