...
def _processar_linha_c198(linha):
    """
    Processa uma única linha do registro C198 e retorna um dicionário.
    
    Formato:
      |C198|NUM_PROC|IND_PROC|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "C198"
    ...
    Uma vez procedida à escrituração do Registro "C198", deve a pessoa jurídica gerar os registros
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |C198|...|)
...
def validar_c198(linhas):
    """
    Valida uma ou mais linhas do registro C198 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_c198(linha)
