...
def _processar_linha_c509(linha):
    """
    Processa uma única linha do registro C509 e retorna um dicionário.
    
    Formato:
      |C509|NUM_PROC|IND_PROC|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "C509"
    ...
    Uma vez procedida à escrituração do Registro "C509", deve a pessoa jurídica gerar os registros
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |C509|...|)
    ...
    resultado = {
        "REG": {
            "titulo": "Registro",
            "valor": reg,
            "descricao": "Texto fixo contendo 'C509'"
        },
        "NUM_PROC": {
...
def validar_c509(linhas):
    """
    Valida uma ou mais linhas do registro C509 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_c509(linha)
