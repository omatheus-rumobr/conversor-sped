...
def _processar_linha_c491(linha):
    """
    Processa uma única linha do registro C491 e retorna um dicionário.
    
    Formato:
      |C491|COD_ITEM|CST_PIS|CFOP|VL_ITEM|VL_BC_PIS|ALIQ_PIS|QUANT_BC_PIS|ALIQ_PIS_QUANT|VL_PIS|COD_CTA|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "C491"
    ...
    Nota: Este registro detalha a consolidação de documentos emitidos por ECF (Códigos 02, 2D, 59 e 60) – PIS/Pasep.
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |C491|...|)
...
def validar_c491(linhas):
    """
    Valida uma ou mais linhas do registro C491 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_c491(linha)
