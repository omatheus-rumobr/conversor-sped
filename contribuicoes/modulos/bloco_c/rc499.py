...
def _processar_linha_c499(linha):
    """
    Processa uma única linha do registro C499 e retorna um dicionário.
    
    Formato:
      |C499|NUM_PROC|IND_PROC|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "C499"
    ...
    Uma vez procedida à escrituração do Registro "C499", deve a pessoa jurídica gerar os registros
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |C499|...|)
    ...
    resultado = {
        "REG": {
            "titulo": "Registro",
            "valor": reg,
            "descricao": "Texto fixo contendo 'C499'"
        },
        "NUM_PROC": {
...
def validar_c499(linhas):
    """
    Valida uma ou mais linhas do registro C499 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_c499(linha)
