...
def _processar_linha_i199(linha):
    """
    Processa uma única linha do registro I199 e retorna um dicionário.
    
    Formato:
      |I199|NUM_PROC|IND_PROC|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "I199"
    ...
    Nota: Registro filho de "I100 – Consolidação das Operações do período", específico para a pessoa
    jurídica informar a existência de processo administrativo ou judicial que autoriza a adoção de
    tratamento tributário (CST), base de cálculo ou alíquota diversa da prevista na legislação.
    Trata-se de informação essencial a ser prestada na escrituração para a adequada validação das
    contribuições apuradas no Registro I100.
    
    Uma vez procedida à escrituração do Registro "I199", deve a pessoa jurídica gerar os registros
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |I199|...|)
...
def validar_i119(linhas):
    """
    Valida uma ou mais linhas do registro I199 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_i199(linha)
