...
def _processar_linha_i001(linha: str) -> Dict[str, Any] | None:
    """
    Processa uma única linha do registro I001 e retorna um dicionário.

    Formato: |I001|IND_MOV|

    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "I001"
    - IND_MOV: obrigatório, valores válidos [0, 1]

    Validação (camada superior):
    - Se IND_MOV = "1": somente registros de abertura e encerramento do bloco (I001 e I990).
    - Se IND_MOV = "0": deve existir pelo menos um registro além de I001 e I990.
    """
...
def _validar_regras_cruzadas_ind_mov_i(
    ind_mov: str,
    linhas_bloco_i: Union[str, List[str], None],
) -> List[str]:
    erros: List[str] = []
    linhas_norm = _normalizar_linhas(linhas_bloco_i)
    if not linhas_norm:
        return erros
    ...
    regs_abertura_encerramento = {"I001", "I990"}
    ...
    if ind_mov == "1":
        ...
        if regs_invalidos:
            erros.append(
                "IND_MOV=1: bloco I não pode conter registros além de I001 e I990. "
                f"Encontrado(s): {', '.join(regs_invalidos)}."
            )
    elif ind_mov == "0":
        if len(regs_outros) == 0:
            erros.append("IND_MOV=0: deve existir pelo menos um registro no bloco I além de I001 e I990.")
...
def validar_i001(
    ...
    *,
    linhas_bloco_i: Union[str, List[str], None] = None,
) -> str:
    """
    Valida uma ou mais linhas do registro I001 do SPED EFD-Contribuições.

    Args:
        linhas: String com uma linha, string com múltiplas linhas separadas por \\n,
                ou lista de strings. Cada linha deve estar no formato:
                |I001|IND_MOV|
        linhas_bloco_i: opcional; linhas do Bloco I para validar a regra cruzada do IND_MOV
    ...
    for linha in linhas_para_processar:
        if _extrair_reg(linha) != "I001":
            continue

        item = _processar_linha_i001(linha)
        if item is None:
            continue

        ind_mov = str(item.get("IND_MOV", {}).get("valor", "") or "")
        erros = _validar_regras_cruzadas_ind_mov_i(ind_mov, linhas_bloco_i) if linhas_bloco_i else []
        if erros:
