...
def _processar_linha_1610(linha):
    """
    Processa uma única linha do registro 1610 e retorna um dicionário.
    
    Formato:
      |1610|CNPJ|CST_COFINS|COD_PART|DT_OPER|VL_OPER|VL_BC_COFINS|ALIQ_COFINS|VL_COFINS|COD_CTA|DESC_COMPL|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "1610"
    - CNPJ: obrigatório, 14 dígitos, validar DV (deve existir no registro 0140)
    - CST_COFINS: obrigatório, 2 dígitos (código conforme Tabela 4.3.4)
    ...
    - VL_BC_COFINS: obrigatório, numérico com 3 decimais, não negativo
    - ALIQ_COFINS: obrigatório, numérico com 4 decimais, não negativo
    - VL_COFINS: obrigatório, numérico com 2 decimais, não negativo
    - COD_CTA: opcional, até 255 caracteres
    - DESC_COMPL: opcional
    
    Nota: A chave deste registro é formada pelos campos: CNPJ + CST_COFINS + COD_PART + DT_OPER + ALIQ_COFINS + COD_CTA.
    A validação de que CNPJ deve existir no registro 0140, COD_PART deve existir no registro 0150,
    e CST_COFINS deve existir na Tabela 4.3.4 deve ser feita em uma camada superior.
    A validação de que a soma dos valores do campo VL_COFINS deve ser transportada para o campo VL_CONT_APUR
    do registro pai 1600 deve ser feita em uma camada superior.
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |1610|...|)
    ...
    cnpj = obter_campo(1)
    cst_cofins = obter_campo(2)
    cod_part = obter_campo(3)
    dt_oper = obter_campo(4)
    vl_oper = obter_campo(5)
    vl_bc_cofins = obter_campo(6)
    aliq_cofins = obter_campo(7)
    ...
    # VL_BC_COFINS: obrigatório, numérico com 3 decimais, não negativo
    ok2, val2, _ = validar_valor_numerico(vl_bc_cofins, decimais=3, obrigatorio=True, nao_negativo=True)
    if not ok2:
        return None
    
    # ALIQ_COFINS: obrigatório, numérico com 4 decimais, não negativo
    ok3, val3, _ = validar_valor_numerico(aliq_cofins, decimais=4, obrigatorio=True, nao_negativo=True)
    if not ok3:
        return None
    
    # VL_COFINS: obrigatório, numérico com 2 decimais, não negativo
    ok4, val4, _ = validar_valor_numerico(vl_cofins, decimais=2, obrigatorio=True, nao_negativo=True)
    if not ok4:
        return None
    ...
    resultado = {
        ...
        "VL_BC_COFINS": {
            "titulo": "Base de cálculo da COFINS (em valor ou em quantidade)",
            "valor": vl_bc_cofins,
            "valor_formatado": fmt_valor_3dec(val2)
        },
        "ALIQ_COFINS": {
            "titulo": "Alíquota da COFINS (em percentual ou em reais)",
            "valor": aliq_cofins,
...
def validar_1610(linhas):
    """
    Valida uma ou mais linhas do registro 1610 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_1610(linha)
