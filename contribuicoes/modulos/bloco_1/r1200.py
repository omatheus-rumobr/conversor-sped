...
def _processar_linha_1200(linha, per_apu_escrit=None):
    """
    Processa uma única linha do registro 1200 e retorna um dicionário.
    
    Formato:
      |1200|PER_APUR_ANT|NAT_CONT_REC|VL_CONT_APUR|VL_CRED_PIS_DESC|VL_CONT_DEV|VL_OUT_DED|VL_CONT_EXT|VL_MUL|VL_JUR|DT_RECOL|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "1200"
    ...
    - VL_CRED_PIS_DESC: obrigatório, numérico com 2 decimais, não negativo
    - VL_CONT_DEV: obrigatório, numérico com 2 decimais, não negativo
      - Deve ser igual a VL_CONT_APUR - VL_CRED_PIS_DESC
    ...
    Nota: A validação de que NAT_CONT_REC deve existir na Tabela 4.3.5 deve ser feita
    em uma camada superior que tenha acesso a essa tabela.
    As validações de correspondência com registros 1210 e 1220 devem ser feitas em uma camada superior.
    ...
    vl_cred_pis_desc = obter_campo(4)
    ...
    # VL_CRED_PIS_DESC: obrigatório, numérico com 2 decimais, não negativo
    ok2, val2, _ = validar_valor_numerico(vl_cred_pis_desc, decimais=2, obrigatorio=True, nao_negativo=True)
    if not ok2:
        return None
    ...
    # Validação: VL_CONT_DEV deve ser igual a VL_CONT_APUR - VL_CRED_PIS_DESC
    ...
    # Validações condicionais: se VL_CONT_EXT > 0, então VL_MUL, VL_JUR e DT_RECOL devem ser informados
    if val5 > 0:
        if not vl_mul or val6 == 0.0:
            # Não é obrigatório ter valor > 0, mas deve estar informado (mesmo que zero)
            pass
        if not vl_jur or val7 == 0.0:
            # Não é obrigatório ter valor > 0, mas deve estar informado (mesmo que zero)
            pass
        if not dt_recol:
            return None
    ...
    resultado = {
        ...
        "VL_CRED_PIS_DESC": {
            "titulo": "Valor do Crédito de PIS/PASEP a Descontar, da Contribuição Social Extemporânea",
...
def validar_1200(linhas, per_apu_escrit=None):
    """
    Valida uma ou mais linhas do registro 1200 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_1200(linha, per_apu_escrit=per_apu_escrit)
