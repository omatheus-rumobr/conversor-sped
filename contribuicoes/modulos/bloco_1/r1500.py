...
def _processar_linha_1500(linha, per_apu_escrit=None):
    """
    Processa uma única linha do registro 1500 e retorna um dicionário.
    
    Formato:
      |1500|PER_APU_CRED|ORIG_CRED|CNPJ_SUC|COD_CRED|VL_CRED_APU|VL_CRED_EXT_APU|VL_TOT_CRED_APU|VL_CRED_DESC_PA_ANT|VL_CRED_PER_PA_ANT|VL_CRED_DCOMP_PA_ANT|SD_CRED_DISP_EFD|VL_CRED_DESC_EFD|VL_CRED_PER_EFD|VL_CRED_DCOMP_EFD|VL_CRED_TRANS|VL_CRED_OUT|SLD_CRED_FIM|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "1500"
    ...
    - SLD_CRED_FIM: obrigatório, numérico com 2 decimais, não negativo
      - Deve ser igual a SD_CRED_DISP_EFD - VL_CRED_DESC_EFD - VL_CRED_PER_EFD - VL_CRED_DCOMP_EFD - VL_CRED_TRANS - VL_CRED_OUT
    
    Nota: A chave deste registro é formada pelos campos: PER_APU_CRED + ORIG_CRED + CNPJ_SUC + COD_CRED.
    A validação de que COD_CRED deve existir na Tabela 4.3.6 deve ser feita em uma camada superior.
    As validações de correspondência com registros M500, M600, 1501, 1502, F800 devem ser feitas em uma camada superior.
    ...
    # SLD_CRED_FIM: obrigatório, numérico com 2 decimais, não negativo
    ok13, val13, _ = validar_valor_numerico(sld_cred_fim, decimais=2, obrigatorio=True, nao_negativo=True)
    if not ok13:
        return None
...
def validar_1500(linhas, per_apu_escrit=None):
    """
    Valida uma ou mais linhas do registro 1500 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_1500(linha, per_apu_escrit=per_apu_escrit)
