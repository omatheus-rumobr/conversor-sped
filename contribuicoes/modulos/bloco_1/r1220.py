...
def _processar_linha_1220(linha, per_apur_ant_1200=None):
    """
    Processa uma única linha do registro 1220 e retorna um dicionário.
    
    Formato:
      |1220|PER_APU_CRED|ORIG_CRED|COD_CRED|VL_CRED|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "1220"
    - PER_APU_CRED: obrigatório, formato mmaaaa, período válido
      - Deve ser anterior ou igual ao período de apuração (PER_APUR_ANT) do registro pai 1200
    ...
    VL_CRED_PIS_DESC do registro pai 1200 deve ser feita em uma camada superior.
    
    Args:
        linha: String com uma linha do SPED
        per_apur_ant_1200: Período de apuração do registro pai 1200 (mmaaaa) - opcional, para validação
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |1220|...|)
    ...
    # PER_APU_CRED deve ser anterior ou igual ao período de apuração do registro pai 1200 (quando informado)
    if per_apur_ant_1200:
        ok_1200, per_1200_tuplo = _validar_periodo_mmaaaa(per_apur_ant_1200)
        if ok_1200 and per_apu_cred_tuplo:
            mes_cred, ano_cred = per_apu_cred_tuplo
            mes_1200, ano_1200 = per_1200_tuplo
            # Deve ser anterior ou igual (mesmo mês/ano ou anterior)
            if ano_cred > ano_1200 or (ano_cred == ano_1200 and mes_cred > mes_1200):
                return None
...
def validar_1220(linhas, per_apur_ant_1200=None):
    """
    Valida uma ou mais linhas do registro 1220 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_1220(linha, per_apur_ant_1200=per_apur_ant_1200)
