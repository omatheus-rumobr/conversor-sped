...
def _processar_linha_1501(linha, per_apu_escrit=None):
    """
    Processa uma única linha do registro 1501 e retorna um dicionário.
    
    Formato:
      |1501|COD_PART|COD_ITEM|COD_MOD|SER|SUB_SER|NUM_DOC|DT_OPER|CHV_NFE|VL_OPER|CFOP|NAT_BC_CRED|IND_ORIG_CRED|CST_COFINS|VL_BC_COFINS|ALIQ_COFINS|VL_COFINS|COD_CTA|COD_CCUS|DESC_COMPL|PER_ESCRIT|CNPJ|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "1501"
    ...
    existir na Tabela 4.1.1, NAT_BC_CRED deve existir na Tabela 4.3.7, CST_COFINS deve existir na Tabela 4.3.4,
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |1501|...|)
    ...
    cst_cofins = obter_campo(13)
    vl_bc_cofins = obter_campo(14)
    aliq_cofins = obter_campo(15)
    ...
    resultado = {
        ...
        "VL_BC_COFINS": {
            "titulo": "Base de Cálculo do Crédito de COFINS (em valor ou em quantidade)",
            "valor": vl_bc_cofins,
            "valor_formatado": fmt_valor_3dec(val2)
        },
        "ALIQ_COFINS": {
            "titulo": "Alíquota do COFINS (em percentual ou em reais)",
            "valor": aliq_cofins,
...
def validar_1501(linhas, per_apu_escrit=None):
    """
    Valida uma ou mais linhas do registro 1501 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_1501(linha, per_apu_escrit=per_apu_escrit)
