...
def _processar_linha_f550(linha):
    """
    Processa uma única linha do registro F550 e retorna um dicionário.
    
    Formato:
      |F550|VL_REC_COMP|CST_PIS|VL_DESC_PIS|VL_BC_PIS|ALIQ_PIS|VL_PIS|CST_COFINS|VL_DESC_COFINS|VL_BC_COFINS|ALIQ_COFINS|VL_COFINS|COD_MOD|CFOP|COD_CTA|INFO_COMPL|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "F550"
    ...
    Este registro tem por objetivo representar a escrituração e tratamento fiscal das receitas auferidas
    no período, independente de seu recebimento ou não, segmentado por Código de Situação Tributária - CST,
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |F550|...|)
...
def validar_f550(linhas):
    """
    Valida uma ou mais linhas do registro F550 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_f550(linha)
