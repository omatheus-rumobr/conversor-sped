...
def _processar_linha_f129(linha):
    """
    Processa uma única linha do registro F129 e retorna um dicionário.
    
    Formato:
      |F129|NUM_PROC|IND_PROC|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "F129"
    ...
    Uma vez procedida à escrituração do Registro "F129", deve a pessoa jurídica gerar os registros
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |F129|...|)
...
def validar_f129(linhas):
    """
    Valida uma ou mais linhas do registro F129 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_f129(linha)
