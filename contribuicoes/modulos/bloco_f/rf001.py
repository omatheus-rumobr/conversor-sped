...
def _processar_linha_f001(linha: str) -> Dict[str, Any] | None:
    """
    Processa uma única linha do registro F001 e retorna um dicionário.

    Formato: |F001|IND_MOV|

    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "F001"
    - IND_MOV: obrigatório, valores válidos [0, 1]

    Validação (camada superior):
    - Se IND_MOV = "1": somente podem ser informados os registros F001 e F990.
    - Se IND_MOV = "0": deve ser informado pelo menos um registro além de F001 e F990.
    """
...
def _validar_regras_cruzadas_ind_mov_f(
    ind_mov: str,
    linhas_bloco_f: Union[str, List[str], None],
) -> List[str]:
    erros: List[str] = []
    linhas_norm = _normalizar_linhas(linhas_bloco_f)
    if not linhas_norm:
        return erros
    ...
    regs_abertura_encerramento = {"F001", "F990"}
    ...
    if ind_mov == "1":
        ...
        if regs_invalidos:
            erros.append(
                "IND_MOV=1: bloco F não pode conter registros além de F001 e F990. "
                f"Encontrado(s): {', '.join(regs_invalidos)}."
            )
    elif ind_mov == "0":
        if len(regs_outros) == 0:
            erros.append("IND_MOV=0: deve existir pelo menos um registro no bloco F além de F001 e F990.")
...
def validar_f001(
    ...
    *,
    linhas_bloco_f: Union[str, List[str], None] = None,
) -> str:
    """
    Valida uma ou mais linhas do registro F001 do SPED EFD-Contribuições.

    Args:
        linhas: String com uma linha, string com múltiplas linhas separadas por \\n,
                ou lista de strings. Cada linha deve estar no formato:
                |F001|IND_MOV|
        linhas_bloco_f: opcional; linhas do Bloco F para validar a regra cruzada do IND_MOV
    ...
    for linha in linhas_para_processar:
        if _extrair_reg(linha) != "F001":
            continue

        item = _processar_linha_f001(linha)
        if item is None:
            continue

        ind_mov = str(item.get("IND_MOV", {}).get("valor", "") or "")
        erros = _validar_regras_cruzadas_ind_mov_f(ind_mov, linhas_bloco_f) if linhas_bloco_f else []
        if erros:
