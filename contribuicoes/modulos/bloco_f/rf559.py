...
def _processar_linha_f559(linha):
    """
    Processa uma única linha do registro F559 e retorna um dicionário.
    
    Formato:
      |F559|NUM_PROC|IND_PROC|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "F559"
    ...
    Uma vez procedida à escrituração do Registro "F559", deve a pessoa jurídica gerar os registros
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |F559|...|)
...
def validar_f559(linhas):
    """
    Valida uma ou mais linhas do registro F559 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_f559(linha)
