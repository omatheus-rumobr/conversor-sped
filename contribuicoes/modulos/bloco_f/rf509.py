...
def _processar_linha_f509(linha):
    """
    Processa uma única linha do registro F509 e retorna um dicionário.
    
    Formato:
      |F509|NUM_PROC|IND_PROC|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "F509"
    ...
    Uma vez procedida à escrituração do Registro "F509", deve a pessoa jurídica gerar os registros
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |F509|...|)
...
def validar_f509(linhas):
    """
    Valida uma ou mais linhas do registro F509 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_f509(linha)
