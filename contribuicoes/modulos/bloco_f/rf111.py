...
def _processar_linha_f111(linha):
    """
    Processa uma única linha do registro F111 e retorna um dicionário.
    
    Formato:
      |F111|NUM_PROC|IND_PROC|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "F111"
    ...
    Uma vez procedida à escrituração do Registro "F111", deve a pessoa jurídica gerar os registros
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |F111|...|)
...
def validar_f111(linhas):
    """
    Valida uma ou mais linhas do registro F111 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_f111(linha)
