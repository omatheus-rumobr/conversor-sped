...
def _processar_linha_f519(linha):
    """
    Processa uma única linha do registro F519 e retorna um dicionário.
    
    Formato:
      |F519|NUM_PROC|IND_PROC|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "F519"
    ...
    Uma vez procedida à escrituração do Registro "F519", deve a pessoa jurídica gerar os registros
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |F519|...|)
...
def validar_f519(linhas):
    """
    Valida uma ou mais linhas do registro F519 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_f519(linha)
