...
def _processar_linha_f500(linha):
    """
    Processa uma única linha do registro F500 e retorna um dicionário.
    
    Formato:
      |F500|VL_REC_CAIXA|CST_PIS|VL_DESC_PIS|VL_BC_PIS|ALIQ_PIS|VL_PIS|CST_COFINS|VL_DESC_COFINS|VL_BC_COFINS|ALIQ_COFINS|VL_COFINS|COD_MOD|CFOP|COD_CTA|INFO_COMPL|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "F500"
    - VL_REC_CAIXA: obrigatório, numérico com 2 decimais
    - CST_PIS: obrigatório, valores válidos [01, 02, 04, 05, 06, 07, 08, 09, 49, 99]
    ...
    - VL_PIS: opcional, numérico com 2 decimais
      - Deve corresponder a VL_BC_PIS * ALIQ_PIS / 100 (validação, se ambos preenchidos)
    - CST_COFINS: obrigatório, valores válidos [01, 02, 04, 05, 06, 07, 08, 09, 49, 99]
    ...
    - VL_COFINS: opcional, numérico com 2 decimais
      - Deve corresponder a VL_BC_COFINS * ALIQ_COFINS / 100 (validação, se ambos preenchidos)
    ...
    presumido, optante pela apuração da contribuição para o PIS/Pasep e da Cofins pelo regime de caixa.
    Este registro tem por objetivo representar a escrituração e tratamento fiscal das receitas recebidas
    no período, segmentado por Código de Situação Tributária - CST, do PIS/Pasep e da Cofins.
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |F500|...|)
    ...
    # VL_REC_CAIXA: obrigatório, numérico com 2 decimais
    ok1, val1, _ = validar_valor_numerico(vl_rec_caixa, decimais=2, obrigatorio=True, nao_negativo=True)
    if not ok1:
        return None
    ...
    ok4, val4, _ = validar_valor_numerico(aliq_pis_normalizada, decimais=4, obrigatorio=False, nao_negativo=True)
    if not ok4:
        return None
    ...
    # Validação: VL_PIS deve corresponder a VL_BC_PIS * ALIQ_PIS / 100 (se ambos preenchidos)
    ...
    # Validação: VL_COFINS deve corresponder a VL_BC_COFINS * ALIQ_COFINS / 100 (se ambos preenchidos)
    ...
    resultado = {
        ...
        "VL_REC_CAIXA": {
            "titulo": "Valor total da receita recebida, referente à combinação de CST e Alíquota",
            "valor": vl_rec_caixa,
        ...
        "VL_DESC_PIS": {
            "titulo": "Valor do desconto / exclusão da base de cálculo",
            "valor": vl_desc_pis,
...
def validar_f500(linhas):
    """
    Valida uma ou mais linhas do registro F500 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_f500(linha)
