...
def _processar_linha_f139(linha):
    """
    Processa uma única linha do registro F139 e retorna um dicionário.
    
    Formato:
      |F139|NUM_PROC|IND_PROC|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "F139"
    ...
    Uma vez procedida à escrituração do Registro "F139", deve a pessoa jurídica gerar os registros
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |F139|...|)
...
def validar_f139(linhas):
    """
    Valida uma ou mais linhas do registro F139 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_f139(linha)
