...
def _processar_linha_f569(linha):
    """
    Processa uma única linha do registro F569 e retorna um dicionário.
    
    Formato:
      |F569|NUM_PROC|IND_PROC|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "F569"
    ...
    Uma vez procedida à escrituração do Registro "F569", deve a pessoa jurídica gerar os registros
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |F569|...|)
...
def validar_f569(linhas):
    """
    Valida uma ou mais linhas do registro F569 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_f569(linha)
