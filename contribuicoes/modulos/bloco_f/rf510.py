...
def _processar_linha_f510(linha):
    """
    Processa uma única linha do registro F510 e retorna um dicionário.
    
    Formato:
      |F510|VL_REC_CAIXA|CST_PIS|VL_DESC_PIS|QUANT_BC_PIS|ALIQ_PIS_QUANT|VL_PIS|CST_COFINS|VL_DESC_COFINS|QUANT_BC_COFINS|ALIQ_COFINS_QUANT|VL_COFINS|COD_MOD|CFOP|COD_CTA|INFO_COMPL|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "F510"
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |F510|...|)
...
def validar_f510(linhas):
    """
    Valida uma ou mais linhas do registro F510 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_f510(linha)
