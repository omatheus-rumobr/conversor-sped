...
def _processar_linha_f211(linha):
    """
    Processa uma única linha do registro F211 e retorna um dicionário.
    
    Formato:
      |F211|NUM_PROC|IND_PROC|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "F211"
    ...
    Uma vez procedida à escrituração do Registro "F211", deve a pessoa jurídica gerar os registros
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |F211|...|)
...
def validar_f211(linhas):
    """
    Valida uma ou mais linhas do registro F211 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_f211(linha)
