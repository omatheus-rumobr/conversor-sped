...
def _processar_linha_a010(linha):
    """
    Processa uma única linha do registro A010 e retorna um dicionário.
    
    Formato:
      |A010|CNPJ|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "A010"
    ...
    as operações e documentos fiscais informados neste bloco. Só devem ser escriturados no Registro A010 os
    estabelecimentos que efetivamente tenham realizado operações de prestação ou de contratação de serviços,
    mediante emissão de documento fiscal, que devam ser escrituradas no Bloco A.
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |A010|...|)
...
def validar_a010(linhas):
    """
    Valida uma ou mais linhas do registro A010 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_a010(linha)
