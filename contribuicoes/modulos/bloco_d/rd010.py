...
def _processar_linha_d010(linha):
    """
    Processa uma única linha do registro D010 e retorna um dicionário.
    
    Formato:
      |D010|CNPJ|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "D010"
    ...
    as operações e documentos fiscais informados neste bloco. Só devem ser escriturados no Registro D010 os
    estabelecimentos que efetivamente tenham realizado as operações especificadas no Bloco D (prestação ou
    contratação), relativas a serviços de transporte de cargas e/ou de passageiros, serviços de comunicação
    e de telecomunicação, mediante emissão de documento fiscal definido pela legislação do ICMS e do IPI,
    que devam ser escrituradas no Bloco D.
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |D010|...|)
...
def validar_d010(linhas):
    """
    Valida uma ou mais linhas do registro D010 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_d010(linha)
