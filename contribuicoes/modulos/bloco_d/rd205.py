...
def _processar_linha_d205(linha):
    """
    Processa uma única linha do registro D205 e retorna um dicionário.
    
    Formato:
      |D205|CST_COFINS|VL_ITEM|VL_BC_COFINS|ALIQ_COFINS|VL_COFINS|COD_CTA|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "D205"
    ...
    alíquota e valor da Cofins, referente às operações de transporte consolidadas em D200.
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |D205|...|)
...
def validar_d205(linhas):
    """
    Valida uma ou mais linhas do registro D205 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_d205(linha)
