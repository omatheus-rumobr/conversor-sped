...
def _processar_linha_d501(linha):
    """
    Processa uma única linha do registro D501 e retorna um dicionário.
    
    Formato:
      |D501|CST_PIS|VL_ITEM|NAT_BC_CRED|VL_BC_PIS|ALIQ_PIS|VL_PIS|COD_CTA|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "D501"
    ...
    Nota: Deve ser escriturado um registro D501 para cada item (serviço de comunicação ou de telecomunicação)
    cuja operação dê direito a crédito, pelo seu valor total ou parcial. Caso em relação a um mesmo item venha
    a ocorrer tratamentos tributários diversos (mais de um CST), deve a pessoa jurídica informar um registro
    D501 para cada CST.
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |D501|...|)
    ...
    # CST_PIS: obrigatório, 2 dígitos, valores válidos
    ...
    # NAT_BC_CRED: opcional, 2 caracteres, valores válidos [03, 13]
    if nat_bc_cred:
        if len(nat_bc_cred) > 2:
            return None
        # Valores válidos conforme documentação: 03, 13
        nat_bc_cred_validos = ["03", "13"]
        if nat_bc_cred not in nat_bc_cred_validos:
            # Nota: A validação de valores válidos pode ser feita em camada superior
            # pois a tabela 4.3.7 pode ter outros valores
            pass
    ...
    resultado = {
        ...
        "VL_ITEM": {
            "titulo": "Valor Total dos Itens (Serviços)",
            "valor": vl_item,
        ...
        "NAT_BC_CRED": {
            ...
            "valor": nat_bc_cred,
            "descricao": descricoes_nat_bc_cred.get(nat_bc_cred, "") if nat_bc_cred else ""
        },
        "VL_BC_PIS": {
...
def validar_d501(linhas):
    """
    Valida uma ou mais linhas do registro D501 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_d501(linha)
