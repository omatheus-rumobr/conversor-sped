...
def _processar_linha_d111(linha):
    """
    Processa uma única linha do registro D111 e retorna um dicionário.
    
    Formato:
      |D111|NUM_PROC|IND_PROC|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "D111"
    ...
    Uma vez procedida à escrituração do Registro "D111", deve a pessoa jurídica gerar os registros
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |D111|...|)
...
def validar_d111(linhas):
    """
    Valida uma ou mais linhas do registro D111 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_d111(linha)
