...
def _processar_linha_d605(linha):
    """
    Processa uma única linha do registro D605 e retorna um dicionário.
    
    Formato:
      |D605|COD_CLASS|VL_ITEM|VL_DESC|CST_COFINS|VL_BC_COFINS|ALIQ_COFINS|VL_COFINS|COD_CTA|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "D605"
    - COD_CLASS: obrigatório, 4 dígitos (código de classificação conforme Tabela 4.4.1)
    - VL_ITEM: obrigatório, numérico com 2 decimais, deve ser > 0
    - VL_DESC: opcional, numérico com 2 decimais (descontos/exclusões da base de cálculo)
    ...
    Nota: Devem ser informadas neste registro as informações relacionadas à determinação da base de cálculo
    e do valor da Cofins, dos valores consolidados no registro D600.
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |D605|...|)
    ...
    if not cod_class or not cod_class.isdigit() or len(cod_class) != 4:
        return None
    
    # VL_ITEM: obrigatório, numérico com 2 decimais, deve ser > 0
    ok1, val1, _ = validar_valor_numerico(vl_item, decimais=2, obrigatorio=True, positivo=True)
    if not ok1:
        return None
    ...
    # VL_BC_COFINS: opcional, numérico com 2 decimais
    ok3, val3, _ = validar_valor_numerico(vl_bc_cofins, decimais=2, obrigatorio=False, nao_negativo=True)
    if not ok3:
        return None
    
    # ALIQ_COFINS: opcional, numérico com 8 dígitos e 4 decimais (percentual)
    # Formato: N(008,4) - até 8 dígitos na parte inteira e 4 decimais
    ok4, val4, _ = validar_valor_numerico(aliq_cofins, decimais=4, obrigatorio=False, nao_negativo=True)
    if not ok4:
        return None
    ...
    resultado = {
        ...
        "VL_DESC": {
            "titulo": "Valor acumulado dos descontos/exclusões da base de cálculo",
            "valor": vl_desc,
        ...
        "VL_BC_COFINS": {
            ...
            "valor": vl_bc_cofins,
            "valor_formatado": fmt_valor(val3) if vl_bc_cofins else ""
        },
        "ALIQ_COFINS": {
            "titulo": "Alíquota da COFINS (em percentual)",
            "valor": aliq_cofins,
            "valor_formatado": fmt_percentual(val4) if aliq_cofins else ""
        },
        "VL_COFINS": {
            "titulo": "Valor da COFINS",
            "valor": vl_cofins,
            "valor_formatado": fmt_valor(val5) if vl_cofins else ""
        },
        "COD_CTA": {
            "titulo": "Código da conta contábil debitada/creditada",
            "valor": cod_cta
        }
    }
...
def validar_d605(linhas):
    """
    Valida uma ou mais linhas do registro D605 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_d605(linha)
