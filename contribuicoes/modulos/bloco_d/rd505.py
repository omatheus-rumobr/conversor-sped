...
def _processar_linha_d505(linha):
    """
    Processa uma única linha do registro D505 e retorna um dicionário.
    
    Formato:
      |D505|CST_COFINS|VL_ITEM|NAT_BC_CRED|VL_BC_COFINS|ALIQ_COFINS|VL_COFINS|COD_CTA|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "D505"
    ...
    Nota: Deve ser escriturado um registro D505 para cada item (serviço de comunicação ou de telecomunicação)
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |D505|...|)
...
def validar_d505(linhas):
    """
    Valida uma ou mais linhas do registro D505 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_d505(linha)
