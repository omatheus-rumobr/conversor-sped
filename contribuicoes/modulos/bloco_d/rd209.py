...
def _processar_linha_d209(linha):
    """
    Processa uma única linha do registro D209 e retorna um dicionário.
    
    Formato:
      |D209|NUM_PROC|IND_PROC|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "D209"
    ...
    Uma vez procedida à escrituração do Registro "D209", deve a pessoa jurídica gerar os registros
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |D209|...|)
...
def validar_d209(linhas):
    """
    Valida uma ou mais linhas do registro D209 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_d209(linha)
