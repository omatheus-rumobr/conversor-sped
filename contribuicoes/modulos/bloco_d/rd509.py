...
def _processar_linha_d509(linha):
    """
    Processa uma única linha do registro D509 e retorna um dicionário.
    
    Formato:
      |D509|NUM_PROC|IND_PROC|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "D509"
    ...
    Uma vez procedida à escrituração do Registro "D509", deve a pessoa jurídica gerar os registros
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |D509|...|)
...
def validar_d509(linhas):
    """
    Valida uma ou mais linhas do registro D509 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_d509(linha)
