...
def _processar_linha_d601(linha):
    """
    Processa uma única linha do registro D601 e retorna um dicionário.
    
    Formato:
      |D601|COD_CLASS|VL_ITEM|VL_DESC|CST_PIS|VL_BC_PIS|ALIQ_PIS|VL_PIS|COD_CTA|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "D601"
    ...
    e do valor da Contribuição para o PIS/Pasep, dos valores consolidados no registro D600.
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |D601|...|)
...
def validar_d601(linhas):
    """
    Valida uma ou mais linhas do registro D601 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_d601(linha)
