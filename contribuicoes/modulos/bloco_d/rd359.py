...
def _processar_linha_d359(linha):
    """
    Processa uma única linha do registro D359 e retorna um dicionário.
    
    Formato:
      |D359|NUM_PROC|IND_PROC|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "D359"
    ...
    Uma vez procedida à escrituração do Registro "D359", deve a pessoa jurídica gerar os registros
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |D359|...|)
...
def validar_d359(linhas):
    """
    Valida uma ou mais linhas do registro D359 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_d359(linha)
