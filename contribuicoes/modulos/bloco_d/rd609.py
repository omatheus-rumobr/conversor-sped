...
def _processar_linha_d609(linha):
    """
    Processa uma única linha do registro D609 e retorna um dicionário.
    
    Formato:
      |D609|NUM_PROC|IND_PROC|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "D609"
    ...
    Uma vez procedida à escrituração do Registro "D609", deve a pessoa jurídica gerar os registros
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |D609|...|)
...
def validar_d609(linhas):
    """
    Valida uma ou mais linhas do registro D609 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_d609(linha)
