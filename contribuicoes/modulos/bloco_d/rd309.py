...
def _processar_linha_d309(linha):
    """
    Processa uma única linha do registro D309 e retorna um dicionário.
    
    Formato:
      |D309|NUM_PROC|IND_PROC|
    
    Regras (manual 1.35):
    - REG: obrigatório, valor fixo "D309"
    ...
    Uma vez procedida à escrituração do Registro "D309", deve a pessoa jurídica gerar os registros
    ...
    partes = linha.split('|')
    # Remove primeiro e último se vazios (formato padrão SPED: |D309|...|)
...
def validar_d309(linhas):
    """
    Valida uma ou mais linhas do registro D309 do SPED EFD-Contribuições.
    ...
    for linha in linhas_para_processar:
        resultado = _processar_linha_d309(linha)
