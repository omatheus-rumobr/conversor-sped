...
def validar_registro_m615(
    ...
    *,
    ...
    """
    Valida regras do Registro M615 (Bloco M, EFD-Contribuições 1.35) conforme trecho do manual.

    Campos/validações implementadas (do trecho em `documentacao_blocos/1.35/bloco_M.md:1439-1477`):
    - REG: obrigatório, valor fixo "M615"
    ...
    if not linha_m615 or not isinstance(linha_m615, str):
        return {"ok": False, "erros": ["Linha do registro M615 inválida/vazia."], "campos": {}, "contexto": {}}

    partes = _split_sped(linha_m615)
    ...
    if reg != "M615":
        erros.append("Campo REG inválido: esperado 'M615'.")
...
def validar_m615(
    ...
    *,
    ...
    """
    Wrapper que retorna JSON (array) e valida cada linha M615 encontrada.
    """
    linhas_para_processar = _normalizar_linhas(linhas)
    resultados: List[Dict[str, Any]] = []

    for linha in linhas_para_processar:
        if _extrair_reg(linha) != "M615":
            continue
        resultados.append(
            validar_registro_m615(
                linha,
                dispensado_ecd=dispensado_ecd,
