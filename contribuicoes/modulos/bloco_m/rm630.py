...
def validar_registro_m630(linha_m630: str) -> Dict[str, Any]:
    """
    Valida regras do Registro M630 (Bloco M, EFD-Contribuições 1.35) conforme trecho do manual.

    Campos/validações implementadas (do trecho em `documentacao_blocos/1.35/bloco_M.md:1537-1564`):
    - REG: obrigatório, valor fixo "M630"
    ...
    if not linha_m630 or not isinstance(linha_m630, str):
        return {"ok": False, "erros": ["Linha do registro M630 inválida/vazia."], "campos": {}, "contexto": {}}

    partes = _split_sped(linha_m630)
    ...
    if reg != "M630":
        erros.append("Campo REG inválido: esperado 'M630'.")
...
def validar_m630(linhas: Union[str, List[str], None]) -> str:
    """
    Wrapper que retorna JSON (array) e valida cada linha M630 encontrada.
    """
    linhas_para_processar = _normalizar_linhas(linhas)
    resultados: List[Dict[str, Any]] = []

    for linha in linhas_para_processar:
        if _extrair_reg(linha) != "M630":
            continue
        resultados.append(validar_registro_m630(linha))
