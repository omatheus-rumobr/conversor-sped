...
def validar_registro_m115(
    ...
    *,
    ...
    """
    Valida regras do Registro M115 (Bloco M, EFD-Contribuições 1.35) conforme trecho do manual.

    Campos/validações implementadas (do trecho em `documentacao_blocos/1.35/bloco_M.md:220-251`):
    - REG: obrigatório, valor fixo "M115"
    ...
    if not linha_m115 or not isinstance(linha_m115, str):
        return {"ok": False, "erros": ["Linha do registro M115 inválida/vazia."], "campos": {}, "contexto": {}}

    partes = _split_sped(linha_m115)
    ...
    if reg != "M115":
        erros.append("Campo REG inválido: esperado 'M115'.")
...
def validar_m115(
    ...
    *,
    ...
    """
    Wrapper que retorna JSON (array) e valida cada linha M115 encontrada.
    """
    linhas_para_processar = _normalizar_linhas(linhas)
    resultados: List[Dict[str, Any]] = []

    for linha in linhas_para_processar:
        if _extrair_reg(linha) != "M115":
            continue
        resultados.append(
            validar_registro_m115(
                linha,
                dispensado_ecd=dispensado_ecd,
