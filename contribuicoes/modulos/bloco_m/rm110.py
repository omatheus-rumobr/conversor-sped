...
def validar_registro_m110(linha_m110: str) -> Dict[str, Any]:
    """
    Valida regras do Registro M110 (Bloco M, EFD-Contribuições 1.35) conforme trecho do manual.

    Campos/validações implementadas (do trecho em `documentacao_blocos/1.35/bloco_M.md:192-219`):
    - REG: obrigatório, valor fixo "M110"
    ...
    if not linha_m110 or not isinstance(linha_m110, str):
        return {"ok": False, "erros": ["Linha do registro M110 inválida/vazia."], "campos": {}, "contexto": {}}

    partes = _split_sped(linha_m110)
    ...
    if reg != "M110":
        erros.append("Campo REG inválido: esperado 'M110'.")
...
def validar_m110(linhas: Union[str, List[str], None]) -> str:
    """
    Wrapper que retorna JSON (array) e valida cada linha M110 encontrada.
    """
    linhas_para_processar = _normalizar_linhas(linhas)
    resultados: List[Dict[str, Any]] = []

    for linha in linhas_para_processar:
        if _extrair_reg(linha) != "M110":
            continue
        resultados.append(validar_registro_m110(linha))
