...
def validar_registro_m220(linha_m220: str) -> Dict[str, Any]:
    """
    Valida regras do Registro M220 (Bloco M, EFD-Contribuições 1.35) conforme trecho do manual.

    Campos/validações implementadas (do trecho em `documentacao_blocos/1.35/bloco_M.md:643-668`):
    - REG: obrigatório, valor fixo "M220"
    ...
    if not linha_m220 or not isinstance(linha_m220, str):
        return {"ok": False, "erros": ["Linha do registro M220 inválida/vazia."], "campos": {}, "contexto": {}}

    partes = _split_sped(linha_m220)
    ...
    if reg != "M220":
        erros.append("Campo REG inválido: esperado 'M220'.")
...
def validar_m220(linhas: Union[str, List[str], None]) -> str:
    """
    Wrapper que retorna JSON (array) e valida cada linha M220 encontrada.
    """
    linhas_para_processar = _normalizar_linhas(linhas)
    resultados: List[Dict[str, Any]] = []

    for linha in linhas_para_processar:
        if _extrair_reg(linha) != "M220":
            continue
        resultados.append(validar_registro_m220(linha))
