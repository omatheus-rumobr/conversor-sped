...
def validar_registro_m300(
    linha_m300: str,
    *,
    periodo_atual_mmaaaa: Optional[str] = None,
) -> Dict[str, Any]:
    """
    Valida regras do Registro M300 (Bloco M, EFD-Contribuições 1.35) conforme trecho do manual.

    Campos/validações implementadas (do trecho em `documentacao_blocos/1.35/bloco_M.md:730-759`):
    - REG: obrigatório, valor fixo "M300"
    ...
    if not linha_m300 or not isinstance(linha_m300, str):
        return {"ok": False, "erros": ["Linha do registro M300 inválida/vazia."], "campos": {}, "contexto": {}}

    partes = _split_sped(linha_m300)
    ...
    if reg != "M300":
        erros.append("Campo REG inválido: esperado 'M300'.")
...
def validar_m300(
    ...
    *,
    ...
    """
    Wrapper que retorna JSON (array) e valida cada linha M300 encontrada.
    """
    linhas_para_processar = _normalizar_linhas(linhas)
    resultados: List[Dict[str, Any]] = []

    for linha in linhas_para_processar:
        if _extrair_reg(linha) != "M300":
            continue
        resultados.append(validar_registro_m300(linha, periodo_atual_mmaaaa=periodo_atual_mmaaaa))
