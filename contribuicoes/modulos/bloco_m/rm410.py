...
def validar_registro_m410(
    linha_m410: str,
    *,
    periodo_atual_mmaaaa: Optional[str] = None,
    dispensado_ecd: bool = False,
    exigir_cod_cta_a_partir_de: Optional[str] = "112017",
    vl_tot_rec_m400: Optional[float] = None,
) -> Dict[str, Any]:
    """
    Valida regras do Registro M410 (Bloco M, EFD-Contribuições 1.35) conforme trecho do manual.

    Campos/validações implementadas (do trecho em `documentacao_blocos/1.35/bloco_M.md:825-856`):
    - REG: obrigatório, valor fixo "M410"
    ...
    - DESC_COMPL: opcional
    - Observação (validação cruzada): soma de VL_REC dos M410 deve corresponder ao VL_TOT_REC do M400.
      Aqui isso pode ser verificado na camada superior; opcionalmente, se `vl_tot_rec_m400` for informado,
      validamos apenas que o VL_REC individual não excede o total do M400.
    """
    erros: List[str] = []

    if not linha_m410 or not isinstance(linha_m410, str):
        return {"ok": False, "erros": ["Linha do registro M410 inválida/vazia."], "campos": {}, "contexto": {}}

    partes = _split_sped(linha_m410)
    ...
    if reg != "M410":
        erros.append("Campo REG inválido: esperado 'M410'.")
    ...
    if vl_tot_rec_m400 is not None and num_vl_rec is not None:
        if num_vl_rec - float(vl_tot_rec_m400) > 1e-6:
            erros.append("VL_REC não pode ser maior que o VL_TOT_REC do registro pai M400.")
    ...
    return {
        ...
        "contexto": {
            ...
            "vl_tot_rec_m400": vl_tot_rec_m400,
        },
    }
...
def validar_m410(
    ...
    *,
    ...
    """
    Wrapper que retorna JSON (array) e valida cada linha M410 encontrada.
    """
    linhas_para_processar = _normalizar_linhas(linhas)
    resultados: List[Dict[str, Any]] = []

    for linha in linhas_para_processar:
        if _extrair_reg(linha) != "M410":
            continue
        resultados.append(
            validar_registro_m410(
                linha,
