...
def validar_registro_m515(
    ...
    *,
    ...
    """
    Valida regras do Registro M515 (Bloco M, EFD-Contribuições 1.35) conforme trecho do manual.

    Campos/validações implementadas (do trecho em `documentacao_blocos/1.35/bloco_M.md:1058-1089`):
    - REG: obrigatório, valor fixo "M515"
    ...
    if not linha_m515 or not isinstance(linha_m515, str):
        return {"ok": False, "erros": ["Linha do registro M515 inválida/vazia."], "campos": {}, "contexto": {}}

    partes = _split_sped(linha_m515)
    ...
    if reg != "M515":
        erros.append("Campo REG inválido: esperado 'M515'.")
...
def validar_m515(
    ...
    *,
    ...
    """
    Wrapper que retorna JSON (array) e valida cada linha M515 encontrada.
    """
    linhas_para_processar = _normalizar_linhas(linhas)
    resultados: List[Dict[str, Any]] = []

    for linha in linhas_para_processar:
        if _extrair_reg(linha) != "M515":
            continue
        resultados.append(
            validar_registro_m515(
                linha,
