...
def validar_registro_m510(linha_m510: str) -> Dict[str, Any]:
    """
    Valida regras do Registro M510 (Bloco M, EFD-Contribuições 1.35) conforme trecho do manual.

    Campos/validações implementadas (do trecho em `documentacao_blocos/1.35/bloco_M.md:1030-1057`):
    - REG: obrigatório, valor fixo "M510"
    ...
    if not linha_m510 or not isinstance(linha_m510, str):
        return {"ok": False, "erros": ["Linha do registro M510 inválida/vazia."], "campos": {}, "contexto": {}}

    partes = _split_sped(linha_m510)
    ...
    if reg != "M510":
        erros.append("Campo REG inválido: esperado 'M510'.")
...
def validar_m510(linhas: Union[str, List[str], None]) -> str:
    """
    Wrapper que retorna JSON (array) e valida cada linha M510 encontrada.
    """
    linhas_para_processar = _normalizar_linhas(linhas)
    resultados: List[Dict[str, Any]] = []

    for linha in linhas_para_processar:
        if _extrair_reg(linha) != "M510":
            continue
        resultados.append(validar_registro_m510(linha))
