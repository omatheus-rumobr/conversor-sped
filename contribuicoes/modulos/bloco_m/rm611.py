...
def validar_registro_m611(linha_m611: str) -> Dict[str, Any]:
    """
    Valida regras do Registro M611 (Bloco M, EFD-Contribuições 1.35) conforme trecho do manual.

    Campos/validações implementadas (do trecho em `documentacao_blocos/1.35/bloco_M.md:1407-1438`):
    - REG: obrigatório, valor fixo "M611"
    ...
    if not linha_m611 or not isinstance(linha_m611, str):
        return {"ok": False, "erros": ["Linha do registro M611 inválida/vazia."], "campos": {}, "contexto": {}}

    partes = _split_sped(linha_m611)
    ...
    if reg != "M611":
        erros.append("Campo REG inválido: esperado 'M611'.")
...
def validar_m611(linhas: Union[str, List[str], None]) -> str:
    """
    Wrapper que retorna JSON (array) e valida cada linha M611 encontrada.
    """
    linhas_para_processar = _normalizar_linhas(linhas)
    resultados: List[Dict[str, Any]] = []

    for linha in linhas_para_processar:
        if _extrair_reg(linha) != "M611":
            continue
        resultados.append(validar_registro_m611(linha))
