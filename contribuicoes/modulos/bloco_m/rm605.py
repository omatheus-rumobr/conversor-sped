...
def validar_registro_m605(
    linha_m605: str,
    *,
    total_debito_para_o_campo: Optional[float] = None,
) -> Dict[str, Any]:
    """
    Valida regras do Registro M605 (Bloco M, EFD-Contribuições 1.35) conforme trecho do manual.

    Campos/validações implementadas (do trecho em `documentacao_blocos/1.35/bloco_M.md:1160-1186`):
    - REG: obrigatório, valor fixo "M605"
    - NUM_CAMPO: obrigatório, deve referenciar o campo do M600 detalhado (Campo 08 ou Campo 12)
    ...
    - Validação (opcional): se `total_debito_para_o_campo` for informado, VL_DEBITO deve ser <= esse total
      (a validação do somatório de vários M605 contra o M600 deve ser feita na camada que tem acesso a todas as linhas)
    """
    erros: List[str] = []

    if not linha_m605 or not isinstance(linha_m605, str):
        return {"ok": False, "erros": ["Linha do registro M605 inválida/vazia."], "campos": {}, "contexto": {}}

    partes = _split_sped(linha_m605)
    ...
    if reg != "M605":
        erros.append("Campo REG inválido: esperado 'M605'.")
    ...
    if not num_campo:
        ...
        if nc not in {"08", "12"}:
            erros.append("Campo NUM_CAMPO inválido: valores esperados '08' (campo 08 do M600) ou '12' (campo 12 do M600).")
        num_campo = nc
    ...
    if total_debito_para_o_campo is not None and num_vl_debito is not None:
        if num_vl_debito - float(total_debito_para_o_campo) > 1e-6:
            erros.append("VL_DEBITO não pode ser maior que o total a recolher do campo detalhado no M600.")
...
def validar_m605(
    ...
    *,
    ...
    """
    Wrapper que retorna JSON (array) e valida cada linha M605 encontrada.
    """
    linhas_para_processar = _normalizar_linhas(linhas)
    resultados: List[Dict[str, Any]] = []

    for linha in linhas_para_processar:
        if _extrair_reg(linha) != "M605":
            continue
        resultados.append(validar_registro_m605(linha, total_debito_para_o_campo=total_debito_para_o_campo))
