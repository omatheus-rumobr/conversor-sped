...
def validar_registro_m800(
    ...
    *,
    ...
    """
    Valida regras do Registro M800 (Bloco M, EFD-Contribuições 1.35) conforme trecho do manual.

    Campos/validações implementadas (do trecho em `documentacao_blocos/1.35/bloco_M.md:1594-1634`):
    - REG: obrigatório, valor fixo "M800"
    - CST_COFINS: obrigatório, valores válidos ["04","05","06","07","08","09"]
    ...
    if not linha_m800 or not isinstance(linha_m800, str):
        return {"ok": False, "erros": ["Linha do registro M800 inválida/vazia."], "campos": {}, "contexto": {}}

    partes = _split_sped(linha_m800)
    reg = _obter_campo(partes, 0)
    cst_cofins = _obter_campo(partes, 1)
    vl_tot_rec = _obter_campo(partes, 2)
    cod_cta = _obter_campo(partes, 3)
    desc_compl = _obter_campo(partes, 4)

    if reg != "M800":
        erros.append("Campo REG inválido: esperado 'M800'.")

    valores_validos_cst = {"04", "05", "06", "07", "08", "09"}
    if not cst_cofins:
        erros.append("Campo CST_COFINS obrigatório não informado.")
    elif cst_cofins not in valores_validos_cst:
        erros.append("Campo CST_COFINS inválido: valores válidos são 04, 05, 06, 07, 08, 09.")
...
def validar_m800(
    ...
    *,
    ...
    """
    Wrapper que retorna JSON (array) e valida cada linha M800 encontrada.
    """
    linhas_para_processar = _normalizar_linhas(linhas)
    resultados: List[Dict[str, Any]] = []

    for linha in linhas_para_processar:
        if _extrair_reg(linha) != "M800":
            continue
        resultados.append(
            validar_registro_m800(
                linha,
