...
def validar_registro_m200(linha_m200: str) -> Dict[str, Any]:
    """
    Valida regras do Registro M200 (Bloco M, EFD-Contribuições 1.35) conforme trecho do manual.

    Campos do layout (do trecho em `documentacao_blocos/1.35/bloco_M.md:252-320`):
    - 01 REG (obrigatório, "M200")
    ...
    - 05 VL_TOT_CONT_NC_DEV (obrigatório, numérico)
    - 06 VL_RET_NC (obrigatório, numérico)
    - 07 VL_OUT_DED_NC (obrigatório, numérico)
    - 08 VL_CONT_NC_REC (obrigatório, numérico)
    - 09 VL_TOT_CONT_CUM_PER (obrigatório, numérico)
    - 10 VL_RET_CUM (obrigatório, numérico)
    - 11 VL_OUT_DED_CUM (obrigatório, numérico)
    - 12 VL_CONT_CUM_REC (obrigatório, numérico)
    - 13 VL_TOT_CONT_REC (obrigatório, numérico)

    Validações explícitas do manual (quando aplicável):
    - (03 + 04) <= 02
    - 06 <= 05
    - 10 <= 09
    - Consistências de cálculo (se todos os campos envolvidos estiverem válidos):
      - 05 == 02 - 03 - 04
      - 08 == 05 - 06 - 07
      - 12 == 09 - 10 - 11
      - 13 == 08 + 12
    ...
    if not linha_m200 or not isinstance(linha_m200, str):
        return {"ok": False, "erros": ["Linha do registro M200 inválida/vazia."], "campos": {}, "contexto": {}}

    partes = _split_sped(linha_m200)
    ...
    if reg != "M200":
        erros.append("Campo REG inválido: esperado 'M200'.")
    ...
    # Validações do manual (se der para avaliar)
    ...
    # Consistências aritméticas (tolerância)
...
def validar_m200(linhas: Union[str, List[str], None]) -> str:
    """
    Wrapper que retorna JSON (array) e valida cada linha M200 encontrada.
    """
    linhas_para_processar = _normalizar_linhas(linhas)
    resultados: List[Dict[str, Any]] = []

    for linha in linhas_para_processar:
        if _extrair_reg(linha) != "M200":
            continue
        resultados.append(validar_registro_m200(linha))
