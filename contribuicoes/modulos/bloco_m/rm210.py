...
def validar_registro_m210(linha_m210: str) -> Dict[str, Any]:
    """
    Valida regras do Registro M210 (Bloco M, EFD-Contribuições 1.35) conforme trecho do manual.
    ...
    - **A partir de 01/01/2019** (campos 01..16)
      VL_BC_CONT_AJUS (campo 07) = VL_BC_CONT + VL_AJUS_ACRES_BC_PIS - VL_AJUS_REDUC_BC_PIS
    ...
    if not linha_m210 or not isinstance(linha_m210, str):
        return {"ok": False, "erros": ["Linha do registro M210 inválida/vazia."], "campos": {}, "contexto": {}}

    partes = _split_sped(linha_m210)
    reg = _obter_campo(partes, 0)
    if reg != "M210":
        erros.append("Campo REG inválido: esperado 'M210'.")

    # Detecta leiaute pelo número de campos
    # (REG + 12) => 13 campos (até 2018); (REG + 15) => 16 campos (a partir de 2019)
    layout: str
    if len(partes) >= 16:
        layout = "2019+"
    elif len(partes) >= 13:
        ...
        layout = "desconhecido"
        erros.append("Linha M210 incompleta: quantidade de campos insuficiente para validar.")
    ...
    if layout == "ate_2018":
        ...
        # Obrigatórios numéricos: VL_REC_BRT, VL_BC_CONT, VL_CONT_APUR, VL_AJUS_ACRES, VL_AJUS_REDUC, VL_CONT_PER
        obrig = ["VL_REC_BRT", "VL_BC_CONT", "VL_CONT_APUR", "VL_AJUS_ACRES", "VL_AJUS_REDUC", "VL_CONT_PER"]
        # Opcionais numéricos: ALIQ_PIS, QUANT_BC_PIS, ALIQ_PIS_QUANT, VL_CONT_DIFER, VL_CONT_DIFER_ANT
        opc = ["ALIQ_PIS", "QUANT_BC_PIS", "ALIQ_PIS_QUANT", "VL_CONT_DIFER", "VL_CONT_DIFER_ANT"]
        ...
        # Fórmula: VL_CONT_PER = VL_CONT_APUR + VL_AJUS_ACRES - VL_AJUS_REDUC - VL_CONT_DIFER + VL_CONT_DIFER_ANT
        ...
        campos.update(
            {
                ...
                "VL_REC_BRT": _obter_campo(partes, 2),
                "VL_BC_CONT": _obter_campo(partes, 3),
                "VL_AJUS_ACRES_BC_PIS": _obter_campo(partes, 4),
                "VL_AJUS_REDUC_BC_PIS": _obter_campo(partes, 5),
                "VL_BC_CONT_AJUS": _obter_campo(partes, 6),
                "ALIQ_PIS": _obter_campo(partes, 7),
                "QUANT_BC_PIS": _obter_campo(partes, 8),
                "ALIQ_PIS_QUANT": _obter_campo(partes, 9),
        ...
        # Obrigatórios numéricos: VL_REC_BRT, VL_BC_CONT, VL_AJUS_ACRES_BC_PIS, VL_AJUS_REDUC_BC_PIS,
        # VL_BC_CONT_AJUS, VL_CONT_APUR, VL_AJUS_ACRES, VL_AJUS_REDUC, VL_CONT_PER
        obrig = [
            "VL_REC_BRT",
            "VL_BC_CONT",
        ...
        # Fórmula BC ajustada: VL_BC_CONT_AJUS = VL_BC_CONT + VL_AJUS_ACRES_BC_PIS - VL_AJUS_REDUC_BC_PIS
        v_bc = nums.get("VL_BC_CONT")
        v_acres_bc = nums.get("VL_AJUS_ACRES_BC_PIS")
        v_reduc_bc = nums.get("VL_AJUS_REDUC_BC_PIS")
        ...
        if None not in (v_bc, v_acres_bc, v_reduc_bc, v_bc_ajus):
            ...
            if not _eq(v_bc_ajus, esperado):
                erros.append(
                    "Consistência: VL_BC_CONT_AJUS deve ser igual a (VL_BC_CONT + VL_AJUS_ACRES_BC_PIS - VL_AJUS_REDUC_BC_PIS)."
                )

        # Fórmula contribuição: VL_CONT_PER = VL_CONT_APUR + VL_AJUS_ACRES - VL_AJUS_REDUC - VL_CONT_DIFER + VL_CONT_DIFER_ANT
        v_cont_per = nums.get("VL_CONT_PER")
        v_cont_apur = nums.get("VL_CONT_APUR")
        v_acres = nums.get("VL_AJUS_ACRES")
        v_reduc = nums.get("VL_AJUS_REDUC")
...
def validar_m210(linhas: Union[str, List[str], None]) -> str:
    """
    Wrapper que retorna JSON (array) e valida cada linha M210 encontrada.
    """
    linhas_para_processar = _normalizar_linhas(linhas)
    resultados: List[Dict[str, Any]] = []

    for linha in linhas_para_processar:
        if _extrair_reg(linha) != "M210":
            continue
        resultados.append(validar_registro_m210(linha))
