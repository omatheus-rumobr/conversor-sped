...
def extrair_bloco_e_subbloco(linha):
    ...
    # Remove o pipe inicial se existir e divide por pipes
    # Isso lida com casos como "||0000|..." onde há pipes duplicados no início
    partes = [p.strip() for p in linha.split('|') if p.strip()]
    
    if not partes:
        return None, None
    
    # O primeiro elemento não vazio após o split deve ser o código do registro
...
def ler_e_separar_sped(arquivo_entrada, pasta_saida='blocos_separados'):
    ...
    """
    # Obtém o diretório onde está o arquivo de entrada
    diretorio_arquivo = os.path.dirname(os.path.abspath(arquivo_entrada))
    # Cria o caminho completo da pasta de saída no mesmo diretório do arquivo
...
if __name__ == "__main__":
    # Obtém o diretório onde está este script
    diretorio_script = Path(__file__).parent
    # Identifica o arquivo SPED na mesma pasta do script
